...
import {
  getIngressRoutesGlobalConfig,
  saveIngressGlobalRequestHeaders,
  saveIngressGlobalResponseHeaders,
  loadIngressRoutesGlobalConfig,
    ...
    if (projectID) {
      ...
      loadIngressRoutesGlobalConfig(projectID)
        .catch((ex) =>
          notify(
            "error",
            "Error fetching global settings of ingress routes",
            ex
          )
        )
    ...
    saveIngressGlobalRequestHeaders(projectID, headers)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Saved the headers config successfully"
        )
      )
          ...
          <Row>
            <Col lg={{ span: 20 }} sm={{ span: 24 }}>
              <FormItemLabel
                ...
                description="These modifications will be made to each ingress request"
              />
              <Headers
                headers={headers}
                handleSubmit={setRequestHeaders}
                loading={loading}
              />
              <Divider />
              <FormItemLabel
                name="Modify response headers"
                description="These modifications will be made to the response of each ingress request"
              />
              <Headers
                headers={resHeaders}
                handleSubmit={setResponseHeaders}
                loading={loading}
              />
            </Col>
          </Row>

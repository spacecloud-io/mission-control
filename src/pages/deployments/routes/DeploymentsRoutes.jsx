...
const DeploymentsRoutes = () => {
  ...
  useEffect(() => {
    ...
    loadServiceRoutes(projectID)
      .catch((ex) =>
        notify(
          "error",
          "Error fetching routes. This page is only available for Kubernetes cluster",
          ex.toString()
        )
      )
  ...
  const handleEditClick = (serviceId, port) => {
    const routeConfig = serviceRoutes[serviceId].find(
      (obj) => obj.source.port === port
    );
    setRouteClicked({
      serviceId: serviceId,
      routeConfig: { port: port, targets: routeConfig.targets },
    });
      ...
      saveServiceRoutes(projectID, serviceId, routeConfig)
        .then(({ queued }) => {
          notify(
            "success",
            "Success",
            queued ? actionQueuedMessage : "Saved service routes successfully"
          );
          resolve();
        })
        .catch((ex) => {
    ...
    deleteServiceRoutes(projectID, serviceId, port)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Deleted service route successfully"
        )
      )
  ...
  const servicePanelHeader = (serviceId, routes) => {
    ...
    return (
      <span>
        {serviceId}
        <span style={{ marginLeft: 8, color: "rgba(0,0,0,0.56)" }}>
          ({rulesText})
        </span>
      </span>
    );
    ...
    <React.Fragment>
        ...
        <div style={{ padding: "32px 32px 0" }}>
          {noOfServices === 0 && (
            <div className="panel">
              ...
              >
                No service routing rules yet. You need to first deploy a
                service.
              </p>
            </div>
          )}
          {noOfServices > 0 && (
            ...
            >
              {Object.entries(serviceRoutes).map(([serviceId, routes]) => (
                <Panel
                  header={servicePanelHeader(serviceId, routes)}
                  key={serviceId}
                >
                  <div>
                    <div>
                    ...
                    <Table
                      ...
                      dataSource={routes.map((obj) => ({
                        serviceId,
                        port: obj.source.port,
                        targets: obj.targets.length,
                      }))}
                    />
                  </div>
                </Panel>
      ...
      {modalVisible && (
        <RoutingModal
          ...
          handleSubmit={(values) =>
            handleSubmit(routeClicked.serviceId, values)
          }
        />
      )}
    </React.Fragment>

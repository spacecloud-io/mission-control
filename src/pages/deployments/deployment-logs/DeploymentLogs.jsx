...
const DeploymentLogs = (props) => {
  ...
  const fetchLogs = (task, replica) => {
    let result = "";
    const promise = client.deployments.fetchDeploymentLogs(
      projectID,
      task,
      replica,
      token,
  ...
  Object.entries(deploymentStatus).forEach(([serviceID, serviceObj = {}]) => {
    options.push({
      ...
      children: Object.keys(serviceObj).map((version) => {
        const replicas =
          serviceObj[version] && serviceObj[version].replicas
            ? serviceObj[version].replicas
            : [];
        return {
          value: version,
          label: version,
          children: replicas.map((replica) => {
            const index = deployments.findIndex(
              (val) => val.id === serviceID && val.version === version
            );
      ...
      <ProjectPageLayout>
        ...
        <Content style={{ display: "flex", flexDirection: "column" }}>
          ...
          <Alert
            message={
              logsCompleted
                ? "The logs stream is closed. Checkout the replica status to make sure the replica is still up."
                : "Logs are streamed here in realtime."
            }
            type="info"
            showIcon
          />
          {logs && (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(logs) }}
            ></div>
          )}
          {!logs && (

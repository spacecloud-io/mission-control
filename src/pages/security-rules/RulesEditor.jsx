...
import {
  ...
  loadSecurityRules,
  saveSecurityRule,
} from "../../operations/securityRuleBuilder";
import {
  securityRuleGroups,
  defaultDBRules,
...
const RulesEditor = () => {
  ...
  // Global state
  const { rule: initialRule, name } = useSelector((state) =>
    getSecurityRuleInfo(state, ruleType, id, group)
  );
  ...
  const rulesChanged =
    activeTab === "builder"
      ? JSON.stringify(initialRule) !== JSON.stringify(rule)
      : stringifiedRule !== initialRuleStringified;
  const defaultRulesPossible =
    ruleType === securityRuleGroups.DB_COLLECTIONS ||
    ruleType === securityRuleGroups.DB_PREPARED_QUERIES;
  ...
  const UseDefaultRulesButton = () => {
    if (!defaultRulesPossible) {
      return (
        <Tooltip title="Default rules work only for database">
          <Button style={{ marginRight: 16 }} disabled>
            Use default rules
          </Button>
        </Tooltip>
      );
    ...
    return (
      <Button onClick={handleUseDefaultRules} style={{ marginRight: 16 }}>
        Use default rules
      </Button>
    );
  };
  ...
  return (
    <React.Fragment>
      ...
      <div className="editor-page">
          ...
          >
            ...
            <Tabs.TabPane tab="JSON" key="JSON">
              <React.Fragment>
                ...
                >
                  <JSONEditor
                    rule={stringifiedRule}
                    setRule={setStringifiedRule}
                  />
                </div>
                <Button
                  type="primary"
                  size="large"
                  onClick={() => onSaveChanges("json")}
                  block
                  style={{ marginTop: 16 }}
                  disabled={!rulesChanged}
                >
          ...
          <div style={{ padding: 32 }}>
            ...
            <div className="rabbit">
              ...
              <p style={{ margin: "30px 0px" }}>
                ...
                {defaultRulesPossible
                  ? "Default rules are being applied."
                  : "Define rules to secure the resource."}
              </p>
              <Button
                onClick={addDefaultSecurityRules}
                type="primary"
                size="large"
              >
                Add security rules
        ...
        {shortcutsDrawer && (
          <ShortcutsDrawer onClose={() => openShortcutsDrawer(false)} />
        )}
      </div>
    </React.Fragment>
  );

...
const LicenseSettings = () => {
  ...
  const {
    ...
    nextRenewal,
    quotas,
  } = useSelector((state) => getEnv(state));
  const clusterUpgraded = useSelector((state) => isClusterUpgraded(state));

  const handleOpenApplyLicensePage = () =>
    history.push(
      `/mission-control/projects/${projectID}/settings/apply-license`
    );
          ...
          <Row>
            <Col lg={{ span: 12 }}>
              <License
                ...
                handleApplyLicense={handleOpenApplyLicensePage}
                handleGetLicense={openBillingPortal}

    ...
    saveClusterSetting("letsencryptEmail", newEmail)
      .then(({ queued }) =>
        notify(
          ...
          queued
            ? actionQueuedMessage
            : "Changed letsencrypt email successfully"
        )
      )
    ...
    saveClusterSetting("enableTelemetry", newTelemetry)
      .then(({ queued }) =>
        notify(
          ...
          queued
            ? actionQueuedMessage
            : `${newTelemetry ? "Enabled" : "Disabled"} telemetry successfully`
        )
      )
      .catch((ex) =>
        notify(
          "error",
          `Error ${newTelemetry ? "enabling" : "disabling"} telemetry`,
          ex
        )
      )
      .finally(() => decrementPendingRequests());
          ...
          <Row>
            <Col lg={{ span: 12 }}>
              <LetsEncryptEmail
                ...
                loading={loading}
                handleSubmit={handleLetsEncryptEmail}
              />
              <Divider />
              <Telemetry
                telemetry={telemetry}
                loading={loading}
                handleSubmit={handleTelemetry}
              />
            </Col>
          </Row>
        </Content>

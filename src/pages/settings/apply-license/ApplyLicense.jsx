...
const ApplyLicense = () => {
    ...
    applyClusterLicense(clusterName, licenseKey, licenseValue)
      .then(() => {
        notify(
          "success",
          "Success",
          "Applied license key to cluster successfully"
        );
        history.goBack();
      })
      .catch((ex) =>
        notify("error", "Error applying license key to cluster", ex)
      )
  ...
  const handleApplyOfflineLicense = (licenseKey) => {
    incrementPendingRequests();
    applyOfflineClusterLicense(licenseKey)
      .then(() => {
        notify(
          "success",
          "Success",

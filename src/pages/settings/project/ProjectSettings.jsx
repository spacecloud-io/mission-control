...
import {
  loadLetsEncryptConfig,
  saveWhiteListedDomains,
  getWhiteListedDomains,
} from "../../../operations/letsencrypt";
import {
  saveAesKey,
  saveDockerRegistry,
  saveContextTimeGraphQL,
  deleteProject,
  addSecret,
  changePrimarySecret,
  removeSecret,
...
const SecretConfigure = lazyWithPreload(() =>
  import("../../../components/settings/project/SecretConfigure")
);
      ...
      addSecret(projectID, values)
        .then(({ queued }) => {
          notify(
            "success",
            "Success",
            queued ? actionQueuedMessage : "Added new secret successfully"
          );
          resolve();
        })
        .catch((ex) => {
    ...
    changePrimarySecret(projectID, index)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Changed primary secret successfully"
        )
      )
    ...
    removeSecret(projectID, index)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Removed secret successfully"
        )
      )
    ...
    saveAesKey(projectID, aesKey)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Changed AES key successfully"
        )
      )
    ...
    saveDockerRegistry(projectID, dockerRegistry)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Changed docker registry successfully"
        )
      )
    ...
    saveContextTimeGraphQL(projectID, contextTimeGraphQL)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Changed context time successfully"
        )
      )
    ...
    saveWhiteListedDomains(projectID, domains)
      .then(({ queued }) =>
        notify(
          ...
          queued
            ? actionQueuedMessage
            : "Saved whitelisted domains successfully"
        )
      )
          ...
          <Row>
            <Col lg={{ span: 12 }}>
              <DockerRegistry
                ...
                handleSubmit={handleDockerRegistry}
              />
              <Divider />
              <SecretConfigure
                secrets={secrets}
              ...
              <AESConfigure
                aesKey={aesKey}
                handleSubmit={handleAES}
                loading={loading}
              />
              <Divider />
              <GraphQLTimeout
                contextTimeGraphQL={contextTimeGraphQL}
                handleSubmit={handleContextTimeGraphQL}
                loading={loading}
              />
              <Divider />
              <WhitelistedDomains
                domains={domains}
                handleSubmit={handleDomains}
                loading={loading}
              />
              <Divider />

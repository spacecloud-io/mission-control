...
import {
  ...
  getIntegrationConfigPermissions,
  getIntegrationAPIPermissions,
  installIntegration,
} from "../../operations/integrations";
...
const InstallIntegration = () => {
  ...
  const useUploadedIntegration =
    state && state.useUploadedIntegration ? true : false;

  // Global state
  const { name, appUrl, healthCheckUrl } = useSelector((state) =>
    getIntegrationDetails(state, integrationId, useUploadedIntegration)
  );
  const configPermissions = useSelector((state) =>
    getIntegrationConfigPermissions(
      state,
      integrationId,
  ...
  const apiPermissions = useSelector((state) =>
    getIntegrationAPIPermissions(state, integrationId, useUploadedIntegration)
  );
            ...
            client.integrations
              ...
              .then(({ ack, retry, error }) => {
                ...
                if (!ack) {
                  notify(
                    "error",
                    "Error getting integration console ready",
                    error
                  );
                  return;
                }
  ...
  const BackToIntegrationsPageButton = () => {
    return (
      <Button key="back" size="large" onClick={handleBackToIntegrations}>
        ...
        <Row>
          <Col lg={{ span: 16, offset: 4 }} sm={{ span: 24 }}>
            ...
            >
              <center>
                <PlayCircleOutlined
                  style={{ fontSize: "40px", color: "rgba(0,0,0,0.56)" }}
                />
                <p style={{ marginTop: "16px" }}>
                  Once you click the start button, space cloud will run the
                  integration and create all the dependencies that it require.{" "}
                </p>
              </center>
              <Alert
                type="info"
        ...
        <Row>
          <Col lg={{ span: 16, offset: 4 }} sm={{ span: 24 }}>
            ...
            >
              ...
              {installationCompleted && installationQueued && (
                <Result
                  status="success"
                  title="Success"
              ...
              {installationCompleted &&
                ...
                !installationSucceeded && (
                  <Result
                    ...
                    extra={[
                      <RetryInstallationButton />,
                      <BackToIntegrationsPageButton />,
                    ]}
                  />
                )}
              {installationCompleted &&

    ...
    addProject(projectId, projectName)
      .then(({ queued }) => {
        ...
        notify(
          ...
          queued
            ? actionQueuedMessage
            : "Project created successfully with suitable defaults"
        );
      })
      ...
      .then(({ queued, enabledEventing }) => {
        if (!queued) {
          ...
          if (enabledEventing) {
            notify(
              "info",
              "Enabled eventing module",
              "Configured this database to store event logs. Check out the settings in eventing section to change it"
            );
          }
        } else {
        ...
        <React.Fragment>
          <Row>
            ...
            >
              <CreateDatabase
                projectId={projectId}
                handleSubmit={handleAddDatabase}
                ignoreDbAliasCheck
              />
              <center style={{ marginTop: 16 }}>
                <a
                  style={{ color: "rgba(255, 255, 255, 0.6)" }}
                  onClick={handleSkipAddDatabase}
                >
                  Skip for now
                </a>
              </center>

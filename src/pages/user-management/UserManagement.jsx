...
const UserManagement = () => {
  ...
  useEffect(() => {
    ...
    loadUserManConfig(projectID)
      .catch((ex) =>
        notify("error", "Error fetching user management config", ex)
      )
    ...
    saveUserManConfig(projectID, provider, config)
      .then(({ queued }) =>
        notify(
          ...
          queued
            ? actionQueuedMessage
            : "Saved user management config successfully"
        )
      )
            ...
            <Panel
              header={
                <div style={{ padding: "8px 0px 8px 16px" }}>
                  <CollapseHeader icon={mailIcon} desc="Mail" />
                </div>
              }
              key="1"
            >
              <div style={{ paddingLeft: 16 }}>
                <Email
                  ...
                  handleSubmit={(config) =>
                    handleProviderConfig("email", config)
                  }
                />
              </div>
            </Panel>

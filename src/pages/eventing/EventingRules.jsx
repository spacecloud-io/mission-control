...
import {
  deleteEventingSecurityRule,
  saveEventingSecurityRule,
  loadEventingSecurityRules,
  getEventingTriggerRules,
  getEventingSecurityRules,
  getEventingDefaultSecurityRule,
...
const EventingRules = () => {
      ...
      saveEventingSecurityRule(projectID, type, rule)
        .then(({ queued }) => {
          notify(
            ...
            queued
              ? actionQueuedMessage
              : "Added eventing security rule successfully"
          );
          resolve();
        })
        .catch((ex) => {
      ...
      deleteEventingSecurityRule(projectID, type)
        .then(({ queued }) => {
          notify(
            ...
            queued
              ? actionQueuedMessage
              : "Deleted eventing security rule successfully"
          );
          resolve();
        })
        .catch((ex) => {
  ...
  const handleSecureClick = (id) =>
    openSecurityRulesPage(projectID, securityRuleGroups.EVENTING, id);
    ...
    return (
      <div style={{ marginTop: 24 }}>
        <div className="panel">
          ...
          >
            Secruity rules helps you restrict access to your data.
          </p>
        </div>
      </div>
    );
      ...
      render: (_, record) => (
        <span>
          ...
          <Popconfirm
            ...
            onConfirm={() => handleDeleteRule(record.type)}
          >
      ...
      <div className="page-content page-content--no-padding">
        ...
        <div className="event-tab-content">
          <h3 style={{ display: "flex", justifyContent: "space-between" }}>
            Security Rules{" "}
            <Button onClick={() => setAddRuleModalVisible(true)} type="primary">
              Add
            </Button>
          </h3>
          {Object.keys(rules).length > 0 ? (
            <Table
              dataSource={Object.keys(rules).map((val) => ({ type: val }))}
              columns={columns}
            />
          ) : (
            <EmptyState />

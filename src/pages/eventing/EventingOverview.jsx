...
import {
  deleteEventingTriggerRule,
  saveEventingTriggerRule,
...
const EventingOverview = () => {
  ...
  const rulesTableData = Object.entries(rules).map(([id, { type }]) => ({
    id,
    type,
  }));
  const noOfRules = rulesTableData.length;
  const ruleClickedInfo = ruleClicked
    ? { id: ruleClicked, ...rules[ruleClicked] }
    : undefined;
  ...
  const handleTriggerRuleClick = (eventType) => {
    history.push(
      `/mission-control/projects/${projectID}/eventing/queue-event`,
      { eventType }
    );
  };
        ...
        .then(({ queued }) => {
          notify(
            ...
            queued
              ? actionQueuedMessage
              : `${
                  isRulePresent ? "Modified" : "Added"
                } trigger rule successfully`
          );
          resolve();
        })
        .catch((ex) => {
          notify(
            "error",
            `Error ${isRulePresent ? "Modifying" : "Adding"} trigger rule`,
            ex
          );
          reject();
        })
    ...
    deleteEventingTriggerRule(projectID, id)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Deleted trigger rule successfully"
        )
      )
      ...
      render: (_, record) => {
        ...
        return (
          <span>
            ...
            {source === "custom" && (
              <a onClick={() => handleTriggerRuleClick(record.type)}>Trigger</a>
            )}
            <a
              style={{ color: "red" }}
  ...
  const alertMsg = (
    <div>
      ...
      <Link to={`/mission-control/projects/${projectID}/eventing/settings`}>
        Eventing Settings tab
      </Link>
          ...
          <Col lg={{ span: 18, offset: 3 }}>
            <Alert
              ...
              message={`Eventing needs to be configured${
                eventingConfig.enabled ? " properly" : ""
              }`}
          ...
          {noOfRules === 0 && (
            <div>
              <div className="panel">
                ...
                >
                  Trigger asynchronous business logic reliably on any events via
                  the eventing queue in Space Cloud.{" "}
                  <a href="https://docs.spaceuptech.com/microservices/eventing">
                    View Docs.
                  </a>
                </p>
                <Button
                  ...
                  disabled={!eventingConfigured}
                >
                  Add first event trigger
                </Button>
                {dbAlert()}
              </div>
            </div>

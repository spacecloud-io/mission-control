...
const columns = [
  ...
  { title: "Trigger name", dataIndex: "rule_name", key: "rule_name" },
  {
    title: "Invocations",
    key: "invocations",
    render: (record) => <p>{record.invocation_logs.length}</p>,
  },
  {
    title: "Status",
    key: "status",
    render: (record) => getIconByStatus(record.status),
  },
  {
    title: "Created at",
    key: "date",
    render: (record) => <p>{record.event_ts}</p>,
  },
];
...
const EventingLogs = () => {
        ...
        client.eventing
          .fetchEventLogs(
            ...
            previousData.length > 0
              ? previousData[previousData.length - 1].event_ts
              : new Date().toISOString(),
            dbType,
            () => internalToken
          )
          .then((res) => {
            ...
            const eventLogsIDMap = [
              ...new Set(eventLogs.map((obj) => obj._id)),
            ].reduce(
            ...
            dispatch(
              set(
                "eventLogs",
                eventLogs.concat(res.filter((obj) => !eventLogsIDMap[obj._id]))
              )
            );
            resolve();
          })
  ...
  const handleRefresh = () => {
    ...
    client.eventing
      ...
      .then((res) => dispatch(set("eventLogs", res)))
      .catch((ex) =>
        notify("error", "Error refreshing event logs", ex.toString())
      )
      ...
      <div>
        {record.request_payload && (
          <React.Fragment>
            ...
            <JSONView data={record.request_payload} parse />
            <br />
            <br />
          </React.Fragment>
        )}
        ...
        {record.response_body && (
          <React.Fragment>
            ...
            <JSONView data={record.response_body} parse />
            <br />
            <br />
          </React.Fragment>
        )}
        {record.error_msg && (
          <React.Fragment>
            ...
            <JSONView data={record.error_msg} parse />
            <br />
            <br />
          </React.Fragment>
        )}
  ...
  const expandedRowRender = (record) => {
    const columns = [
      ...
      { title: "ID", dataIndex: "_id", key: "_id" },
      {
        title: "Date",
        key: "invocation",
        render: (record) => <p>{record.invocation_time}</p>,
      },
    ];

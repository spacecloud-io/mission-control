...
const QueueEvent = () => {
  ...
  // Global state
  const eventTriggerRules = useSelector((state) =>
    getEventingTriggerRules(state)
  );
  const internalToken = useSelector((state) => getAPIToken(state, projectID));

  // Derived state
  const customEventTypes = Object.values(eventTriggerRules)
    .filter(({ type }) => getEventSourceFromType(type) === "custom")
    .map((obj) => obj.type);
      ...
      triggerCustomEvent(projectID, type, payload, isSynchronous, token)
        .then((data) => {
          notify(
            "success",
            "Success",
            "Event successfully queued to Space Cloud"
          );
          resolve(data);
        })
        .catch((ex) => {
        ...
        <div className="page-content page-content--no-padding">
          ...
          >
            <Button
              ...
              onClick={() =>
                history.push(
                  `/mission-control/projects/${projectID}/eventing/overview`
                )
              }
            >
              <LeftOutlined />
            ...
            <span style={{ marginLeft: 16 }}>Queue Event</span>
          </div>
          <br />
          <div>
            <Row>

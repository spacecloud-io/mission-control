...
function EmptyState() {
  ...
  const activeDB = dbAliasNames.find(
    (dbAliasName) => dbConfig[dbAliasName].enabled
  );
  ...
  if (dbAliasNames.findIndex((db) => db === lastUsedDb) !== -1) {
    return (
      <Redirect
        to={`/mission-control/projects/${projectID}/database/${lastUsedDb}/overview`}
      />
    );
  }
  ...
  if (activeDB) {
    return (
      <Redirect
        to={`/mission-control/projects/${projectID}/database/${activeDB}/overview`}
      />
    );
  }

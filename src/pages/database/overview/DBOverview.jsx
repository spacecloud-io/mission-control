...
const Overview = () => {
  ...
  const connString = useSelector((state) =>
    getDbConnectionString(state, selectedDB)
  );
  const unTrackedCollections = useSelector((state) =>
    getUntrackedCollections(state, selectedDB)
  );
  const unTrackedCollectionsInfo = unTrackedCollections.map((colName) => ({
    name: colName,
  }));
  const trackedCollections = useSelector((state) =>
    getTrackedCollectionsInfo(state, selectedDB)
  );
  ...
  const hostString = connString.includes("secrets.")
    ? connString
    : hostName
    ? `${hostName}:${port}`
    : "";
  const clickedColDetails = trackedCollections.find(
    (obj) => obj.name === clickedCol
  );
  ...
  useEffect(() => {
    if (projectID && selectedDB) {
      ...
      loadDBConnState(projectID, selectedDB)
        .catch((ex) =>
          notify("error", "Error fetching database connection state", ex)
        )
  ...
  const handleBrowseClick = (colName) => {
    ...
    history.push(
      `/mission-control/projects/${projectID}/database/${selectedDB}/browse`
    );
  };
  ...
  const handleSecureClick = (colName) =>
    openSecurityRulesPage(
      projectID,
      securityRuleGroups.DB_COLLECTIONS,
      colName,
      selectedDB
    );
  ...
  const handleViewQueriesClick = (colName) => {
    dispatch(set("uiState.selectedCollection", colName));
    history.push(
      `/mission-control/projects/${projectID}/database/${selectedDB}/queries`
    );
  };
  ...
  const handleUntrackClick = (colName) => {
    untrackCollection(projectID, selectedDB, colName)
      .then(({ queued }) => {
        if (!queued) {
          notify(
            "success",
            "Success",
            `Sucessfully untracked ${colName} collection`
          );
          return;
        }
        notify("success", "Success", actionQueuedMessage);
      ...
      .catch((ex) =>
        notify("error", `Error untracking ${colName} collection`, ex)
      );
  };
    ...
    inspectColSchema(projectID, selectedDB, colName)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Reloaded schema successfully"
        )
      )
    ...
    return new Promise((resolve, reject) => {
      ...
      saveColSchema(projectID, selectedDB, colName, schema)
        .then(({ queued }) => {
          if (!queued) {
            notify(
              "success",
              "Success",
              `${editMode ? "Modified" : "Added"} ${colName} successfully`
            );
            dispatch(set("uiState.selectedCollection", colName));
          } else {
            notify("success", "Success", actionQueuedMessage);
        ...
        .catch((ex) => {
          notify(
            "error",
            `Error ${editMode ? "modifying" : "adding"} ${colName}`,
            ex
          );
          reject();
        })
        ...
        .then(({ queued, connected }) => {
          if (!queued) {
            ...
            notify(
              "error",
              "Connection failed",
              ` Unable to connect to database. Make sure your connection string is correct.`
            );
            reject();
            return;
          }
  ...
  const label =
    selectedDBType === dbTypes.MONGO || selectedDBType === dbTypes.EMBEDDED
      ? "collection"
      : "table";
    ...
    },
    {
      ...
      render: (_, { name, isRealtimeEnabled }) => (
        <Switch
          ...
          onChange={(checked) => handleRealtimeEnabled(name, checked)}
        />
      ),
    },
    {
      ...
      render: (_, { name }) => (
        <span>
          ...
          <a onClick={() => handleViewQueriesClick(name)}>
            View Sample Queries
          </a>
          <a onClick={() => handleReloadSchema(name)}>Reload schema</a>
          <a onClick={() => handleUntrackClick(name)}>Untrack</a>
          <Popconfirm
            title={`This will delete all the data from ${name}. Are you sure?`}
          ...
          <div className="db-tab-content">
            <h3>
              ...
              <a
                ...
                onClick={() => setEditConnModalVisible(true)}
              >
                (Edit)
              </a>
            </h3>
            ...
            {!connected && (
              <div className="empty-state overview-img">
                <div className="empty-state__graphic">
                  <img src={disconnectedImg} alt="" />
                </div>
                <p className="empty-state__description">
                  Oops... Space Cloud could not connect to your database
                </p>
                <p className="empty-state__action-text">
                  Enter the correct connection details of your database
                </p>
                <div className="empty-state__action-bar">
                  <Button
                    className="action-rounded reconnect"
                    type="default"
                    onClick={() => handleEditConnString(connString)}
                  >
                    Reconnect
                  </Button>
                  <Button
              ...
              <React.Fragment>
                <div>
                  <div style={{ marginTop: "32px" }}>
                    <span className="collections">Tracked {label}s</span>
                    <Button
                      style={{ float: "right" }}
                      type="primary"
                  ...
                  <div style={{ marginTop: "32px" }}>
                    <Table
                      ...
                      dataSource={trackedCollections}
                      bordered
                      locale={{
                        emptyText: (
                          <Empty
                            image={Empty.PRESENTED_IMAGE_SIMPLE}
                            description="No tracked tables. Add a table"
                          />
                        ),
                      }}
                    />
                ...
                {unTrackedCollections.length > 0 && (
                  <Row>
                    <Col xl={{ span: 8 }} lg={{ span: 12 }} xs={{ span: 24 }}>
                      <div style={{ marginTop: "32px" }}>
                        <span className="collections">Untracked {label}s</span>
                        <Button
                          style={{ float: "right" }}
                          type="primary"
                          ghost
                          onClick={() =>
                            handleTrackCollections(unTrackedCollections)
                          }
                        >
                          Track All
                        </Button>
                      ...
                      <div style={{ marginTop: "32px" }}>
                        <Table
                          columns={untrackedTableColumns}
                          dataSource={unTrackedCollectionsInfo}
                          pagination={false}
                          bordered
                        />
                      </div>
            ...
            {addColModalVisible && (
              <AddCollectionForm
                ...
                handleSubmit={(colName, schema) =>
                  handleAddCollection(addColFormInEditMode, colName, schema)
                }
              />
            )}
            {editConnModalVisible && (
              <EditConnectionForm

...
const Settings = () => {
  ...
  const dbName = useSelector((state) =>
    getDbName(state, projectID, selectedDB)
  );
  const type = useSelector((state) => getDbType(state, selectedDB));
  const preparedQueriesSupported = useSelector((state) =>
    isPreparedQueriesSupported(state, selectedDB)
  );
    ...
    disableDb(projectID, selectedDB)
      .then(({ queued, disabledEventing }) => {
        if (!queued) {
          ...
          history.push(
            `/mission-control/projects/${projectID}/database/${selectedDB}`
          );
          if (disabledEventing) {
            notify(
              "warn",
              "Warning",
              "Eventing is auto disabled. Enable it by changing eventing db or adding a new db"
            );
          }
          return;
        }
  ...
  const handleConfigureDefaultPreparedQueriesRule = () =>
    openSecurityRulesPage(
      projectID,
      securityRuleGroups.DB_PREPARED_QUERIES,
      "default",
      selectedDB
    );
    ...
    reloadDbSchema(projectID, selectedDB)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Reloaded database schema successfully"
        )
      )
    ...
    modifyDbSchema(projectID, selectedDB)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Modified database schema successfully"
        )
      )
    ...
    changeDbName(projectID, selectedDB, dbName)
      .then(({ queued }) => {
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : `Changed ${msg} setting successfully`
        );
      })
      ...
      <div>
        ...
        <div className="page-content page-content--no-padding">
          ...
          <div className="db-tab-content">
            <FormItemLabel
              name={databaseLabelName}
              description={databaseLabelDescription}
            />
            <Alert
              style={{ width: "720px" }}
              message={
                <div>
                  <b>Note:</b> Changing this won't migrate any existing data
            ...
            <Form
              layout="vertical"
              form={form}
              onFinish={handleChangeDBName}
              style={{ width: 300, marginTop: 16 }}
              initialValues={{ dbName: dbName }}
            >
              <Form.Item
                name="dbName"
                initialValue={dbName}
                rules={[
                  {
                    required: true,
                    message: "Please provide database/schema name",
                  },
                ]}
              >
                <Input placeholder="" />
            ...
            <FormItemLabel
              name="Default rules for tables/collections"
              description="Used when a table/collection doesn’t have a rule specified."
            />
            ...
            {preparedQueriesSupported && (
              <React.Fragment>
                <Divider style={{ margin: "16px 0px" }} />
                <FormItemLabel
                  name="Default rules for prepared queries"
                  description="Used when a prepared query doesn’t have a rule specified."
                />
                <Button onClick={handleConfigureDefaultPreparedQueriesRule}>
                  Configure
                </Button>
              </React.Fragment>
            )}
            ...
            <FormItemLabel
              name="Reload schema"
              description="Refresh Space Cloud schema, typically required if you have changed the underlying database"
            />
            <Button onClick={handleReloadDB}>Reload</Button>
            <Divider style={{ margin: "16px 0px" }} />
            <FormItemLabel
              name="Modify schema"
              description="Modifies database as per Space Cloud schema, typically required if you have dropped or modified the underlying database"
            />
            <Button onClick={handleModifyDB}>Modify</Button>
            <Divider style={{ margin: "16px 0px" }} />
            <FormItemLabel
              name="Disable database"
              description="Disables all access to this database"
            />
            <Popconfirm
              title={
                canDisableDB
                  ? `This will disable all access to this database. Are you sure?`
                  : `Eventing and realtime functionality will be disabled since this is the eventing db. Are you sure?`
              }
            ...
            <FormItemLabel
              name="Remove Config"
              description="Removes the config (schema, rules, etc.) of this database without dropping any tables or database"
            />
            <Popconfirm
              title={
                canDisableDB
                  ? `This will remove the database config and disable all access to this database. Are you sure?`
                  : `Eventing and realtime functionality will be disabled since this is the eventing db. Are you sure?`
              }

...
const PlayButton = ({ handleClick }) => {
  return (
    <Tooltip placement="bottom" title="Play in GraphiQL explorer">
      <PlayCircleOutlined
        style={{ marginRight: "24px", fontSize: "18px", cursor: "pointer" }}
        onClick={handleClick}
      />
    </Tooltip>
  );
...
const Queries = () => {
  ...
  const trackedCollectionNames = useSelector((state) =>
  ...
  useEffect(() => {
    if (
      trackedCollectionNames.length > 0 &&
      (!selectedCol ||
        !trackedCollectionNames.some((col) => col === selectedCol))
    ) {
    ...
    return (
      <div>
        <div className="panel" style={{ margin: 24 }}>
          ...
          >
            Queries tab provide customized documentation on the GraphQL queries
            for a particular table/collection
          </p>
        </div>
        <Row>
          <Col sm={{ span: 24 }} lg={{ span: 16, offset: 4 }}>
            <Alert
              message="Note"
              description={`No tracked tables yet. You first need to add a table/collection in order to see its auto generated GraphQL documentation.`}
              type="info"
              showIcon
            />
          </Col>
      ...
      <div>
        ...
        <Row>
          <Col sm={{ span: 24 }} lg={{ span: 16, offset: 4 }}>
            <Alert
              message="Note"
              description={`You first need to describe the schema for ${selectedDB} in order to see its auto generated GraphQL documentation.`}
              type="info"
              showIcon
            />
          </Col>
  ...
  if (schema) {
    readQuery = stringifyQuery(
      generateSampleQueryDBRead(
        dbSchemaASTs,
        selectedCol,
        selectedDB,
        readOptions.applyFilters,
        readOptions.sort,
        readOptions.skip,
        readOptions.limit
      )
    );
    insertQuery = stringifyQuery(
      generateSampleQueryDBInsert(dbSchemaASTs, selectedCol, selectedDB)
    );
    updateQuery = stringifyQuery(
      generateSampleQueryDBUpdate(
        ...
        selectedCol,
        selectedDB,
        updateOptions.applyFilters,
        updateOptions.upsert
      )
    );
    deleteQuery = stringifyQuery(
      generateSampleQueryDBDelete(
        dbSchemaASTs,
        selectedCol,
        selectedDB,
        deleteOptions.applyFilters
      )
    );
  }
          ...
          <div className="db-tab-content">
            ...
            {trackedCollectionNames.length > 0 && (
              <React.Fragment>
                ...
                >
                  {trackedCollectionNames.map((colName) => (
                    <Select.Option value={colName}>{colName}</Select.Option>
                  ))}
                </Select>
                ...
                {schema && (
                  <React.Fragment>
                    <Alert
                      ...
                      message={
                        <div>
                          <b>Note:</b> These are only few sample queries. Visit{" "}
                          <a
                            href="https://docs.spaceuptech.com/storage/database/"
                            target="_blank"
                          >
                            docs of database module
                          </a>{" "}
                          to learn the full querying capabilities
                        </div>
                      }
                      description=""
                      type="info"
                    ...
                    <p className="query-font-small">
                      Fetch records from {selectedCol} table with filters:
                    </p>
                    <Row gutter={48}>
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        ...
                        >
                          <OptionsPanel>
                            <PlayButton
                              handleClick={() => handlePlay(readQuery)}
                            />
                            <Form
                              initialValues={readOptions}
                              layout="inline"
                              style={{ display: "inline-flex" }}
                              onValuesChange={(_, values) =>
                                setReadOptions(values)
                              }
                            >
                              <Form.Item
                                name="applyFilters"
                    ...
                    <p className="query-font-small">
                      Insert a new record in {selectedCol} table:
                    </p>
                    <Row gutter={48}>
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        <div
                        ...
                        >
                          <OptionsPanel>
                            <PlayButton
                              handleClick={() => handlePlay(insertQuery)}
                            />
                          </OptionsPanel>
                      ...
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        ...
                        >
                          <QueryBlock
                            theme="dark"
                            value={insertQuery.response}
                          />
                        </div>
                      </Col>
                    </Row>
                    <p className="query-font-small" style={{ marginTop: 15 }}>
                      Update an existing record in {selectedCol} table:
                    </p>
                    <Row gutter={48}>
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        <div
                        ...
                        >
                          <OptionsPanel>
                            <PlayButton
                              handleClick={() => handlePlay(updateQuery)}
                            />
                            <Form
                              initialValues={updateOptions}
                              layout="inline"
                              style={{ display: "inline-flex" }}
                              onValuesChange={(_, values) =>
                                setUpdateOptions(values)
                              }
                            >
                              <Form.Item
                                name="applyFilters"
                      ...
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        ...
                        >
                          <QueryBlock
                            theme="dark"
                            value={updateQuery.response}
                          />
                        </div>
                      </Col>
                    </Row>
                    <p className="query-font-small" style={{ marginTop: 15 }}>
                      Delete a record from {selectedCol} table:
                    </p>
                    <Row gutter={48} style={{ marginBottom: 25 }}>
                      <Col lg={{ span: 12 }} md={{ span: 24 }}>
                        ...
                        >
                          <OptionsPanel>
                            <PlayButton
                              handleClick={() => handlePlay(deleteQuery)}
                            />
                            <Form
                              initialValues={deleteOptions}
                              layout="inline"
                              style={{ display: "inline-flex" }}
                              onValuesChange={(_, values) =>
                                setDeleteOptions(values)
                              }
                            >
                              <Form.Item
                                name="applyFilters"

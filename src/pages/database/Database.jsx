...
const Database = () => {
  ...
  // Global state
  const { enabled, type, conn } = useSelector((state) =>
    getDbConfig(state, selectedDB)
  );
      ...
      enableDb(projectID, selectedDB, conn)
        .then(({ queued }) => {
          notify(
            "success",
            "Success",
            queued ? actionQueuedMessage : "Successfully enabled database"
          );
          resolve();
        })
        .catch((ex) => {
  ...
  if (enabled) {
    return (
      <Redirect
        to={`/mission-control/projects/${projectID}/database/${selectedDB}/overview`}
      />
    );
  }
    ...
    <div>
      ...
      <div className="page-content ">
        <div className="panel" style={{ margin: 24 }}>
          ...
          <p className="panel__description" style={{ marginBottom: 0 }}>
            {desc}
          </p>
          <Button
            style={{ marginTop: 16 }}

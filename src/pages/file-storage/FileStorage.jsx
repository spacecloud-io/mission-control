...
import {
  notify,
  getFileStorageProviderLabelFromStoreType,
...
const Rules = () => {
  ...
  const handleFileConfig = () => {
    history.push(
      `/mission-control/projects/${projectID}/file-storage/configure`
    );
  };
  ...
  const handleAddRule = (ruleName, prefix, securityRule) => {
    return new Promise((resolve, reject) => {
      incrementPendingRequests();
      saveFileStoreRule(projectID, ruleName, {
        prefix: prefix,
        rule: securityRule,
      })
        .then(({ queued }) => {
          notify(
            "success",
            "Success",
            queued ? actionQueuedMessage : "Added rule successfully"
          );
          resolve();
        })
        .catch((ex) => {
  ...
  const handleSecureClick = (ruleName) =>
    openSecurityRulesPage(projectID, securityRuleGroups.FILESTORE, ruleName);
      ...
      saveFileStorePrefix(projectID, selectedRuleName, newPrefix)
        .then(({ queued }) => {
          notify(
            "success",
            "Success",
            queued ? actionQueuedMessage : "Saved prefix successfully"
          );
          resolve();
        })
        .catch((ex) => {
        ...
        <div className="page-content">
          {!enabled && (
            <div className="panel">
              ...
              >
                Manage files on scalable storage backend via Space Cloud without
                server side code
              </p>
              <Button
                type="primary action-rounded"
                style={{ marginTop: 16 }}
                onClick={handleFileConfig}
              >
                Get started
              </Button>
            </div>
              ...
              {!connected && (
                <div className="empty-state">
                  ...
                  <p className="empty-state__description">
                    Oops... Space Cloud could not connect to your storage
                    provider
                  </p>
                  <p className="empty-state__action-text">
                    Make sure your connection details are correct
                  </p>
                  <div className="empty-state__action-bar">
                    <Button
                      className="action-rounded"
                      type="default"
                      onClick={() => handleConfig(connConfig)}
                    >
                      Reconnect
                    </Button>
                    <Button

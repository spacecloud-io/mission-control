...
const FileStorageConfig = () => {
  ...
  const {
    ...
    disableSSL,
    forcePathStyle,
  } = useSelector((state) => getFileStoreConfig(state));
  ...
  const formInitialValues = {
    ...
    s3Conn:
      initialStoreType === fileStoreProviders.AMAZON_S3 ? conn : undefined,
    s3Bucket:
      initialStoreType === fileStoreProviders.AMAZON_S3 ? bucket : undefined,
    gcpBucket:
      initialStoreType === fileStoreProviders.GCP_STORAGE ? bucket : undefined,
    endpoint: endpoint,
    ...
    saveFileStoreConfig(projectID, newConfig)
      .then(({ queued }) => {
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Configured file storage successfully"
        );
        form.resetFields();
        history.goBack();
      })
          ...
          <Col offset={6} style={{ marginTop: "2%" }}>
            <Card className="Card-align" style={{ width: 706 }}>
              ...
              >
                ...
                >
                  <RadioCards>
                    <Radio.Button value={fileStoreProviders.LOCAL}>
                      Local File Store
                    </Radio.Button>
                    <Radio.Button value={fileStoreProviders.AMAZON_S3}>
                      Amazon S3
                    </Radio.Button>
                    <Radio.Button value={fileStoreProviders.GCP_STORAGE}>
                      Google Cloud Storage
                    </Radio.Button>
                  </RadioCards>
                </Form.Item>
                <ConditionalFormBlock
                  dependency="storeType"
                  condition={() =>
                    form.getFieldValue("storeType") === fileStoreProviders.LOCAL
                  }
                >
                ...
                <ConditionalFormBlock
                  dependency="storeType"
                  condition={() =>
                    form.getFieldValue("storeType") ===
                    fileStoreProviders.AMAZON_S3
                  }
                >
                  ...
                  <Form.Item
                    name="s3Bucket"
                    rules={[
                      { required: true, message: "Please provide a bucket!" },
                    ]}
                  >
                  ...
                  <Form.Item
                    name="s3Conn"
                    rules={[
                      { required: true, message: "Please provide a region!" },
                    ]}
                    shouldUpdate
                  >
                  ...
                  <FormItemLabel
                    ...
                    description="Credentials file is used to authorize Space Cloud to your bucket"
                  />
                  <Form.Item name="credentials">
                    <Radio.Group>
                      <Radio style={{ display: "block" }} value="secret">
                        Load from Space Cloud Secrets{" "}
                  ...
                  <ConditionalFormBlock
                    ...
                    condition={() =>
                      form.getFieldValue("credentials") === "secret"
                    }
                  >
                    <Alert
                    ...
                    <Form.Item
                      rules={[
                        { required: true, message: "Please provide a secret" },
                      ]}
                      name="secret"
                    >
                      <AutoComplete
                        options={fileSecretsAutoCompleteOptions}
                        style={{ width: "300px" }}
                  ...
                  <ConditionalFormBlock
                    dependency="credentials"
                    condition={() =>
                      form.getFieldValue("credentials") === "direct"
                    }
                  >
                    <Alert
                      ...
                      description={
                        <div>
                          {" "}
                          Make sure that you have stored S3 credentials file at{" "}
                          <b> /root/.aws/credentials </b> of Space Cloud Gateway
                          before saving this configuration. Space Cloud will
                          drectly load the credentials from there.
                        </div>
                      }
                      type="info"
                      showIcon
                  ...
                  >
                    <Collapse.Panel header="Advanced settings" key="advanced">
                      ...
                      <Form.Item name="forcePathStyle" valuePropName="checked">
                        <Checkbox>Force path style</Checkbox>
                      </Form.Item>
                    </Collapse.Panel>
                  </Collapse>
                </ConditionalFormBlock>
                <ConditionalFormBlock
                  ...
                  condition={() =>
                    form.getFieldValue("storeType") ===
                    fileStoreProviders.GCP_STORAGE
                  }
                >
                  <FormItemLabel name="Bucket" />
                  <Form.Item
                  ...
                  >
                    <Alert
                      ...
                      description={
                        <div>
                          {" "}
                          Make sure that you have stored the GCS credentials
                          file in a path accessible to the Space Cloud Gateway
                          and started the Gateway with the{" "}
                          <b> GOOGLE_APPLICATION_CREDENTIALS </b> environment
                          (pointing to the credentials file path) before saving
                          this configuration. Space Cloud will load the
                          credentials from the path specified in this

...
const FileStorageConfig = () => {
    ...
    // Derived properties
    const { storeType, bucket, endpoint, conn, secrets } = getProjectConfig(projects, projectID, "modules.fileStore", {})

    // Handlers
    const handleFinish = (values) => {
        const secret = `secrets.${values.secrets[0]}.${values.secrets[1]}`
    ...
    const splitSecret = secrets.split(".")
    const initialSecrets = [splitSecret[1], splitSecret[2].concat("." + splitSecret[3])]
                    ...
                    <Col offset={6} style={{ marginTop: "2%" }}>
                        <Card className="Card-align" style={{ width: 706 }}>
                            <Form layout="vertical" form={form}
                                initialValues={{ 'storeType': storeType ? storeType : "local", 'conn': conn, 'bucket': bucket, "endpoint": endpoint, "credentials": "secrets", "secrets": initialSecrets }}
                                onFinish={handleFinish}>
                                <FormItemLabel name="Choose storage backend" {...console.log(secrets)} />
                                ...
                                <ConditionalFormBlock dependency="storeType" condition={() => form.getFieldValue("storeType") === "amazon-s3"}>
                                    ...
                                    <Form.Item name="credentials">
                                        <Radio.Group>
                                            <Radio style={{ display: 'block' }} className="radio-align" value="secrets">Load from Space Cloud Secrets <span style={{ fontWeight: 'bold' }}>(recommended method)</span></Radio>
                                            <Radio style={{ display: 'block', marginTop: "3%" }} className="radio-align" value="direct">Load Directly</Radio>
                                        </Radio.Group>
                                    </Form.Item>
                                    <ConditionalFormBlock dependency="credentials" condition={() => form.getFieldValue("credentials") === "secrets"}>
                                        <Alert
                                        ...
                                        <Form.Item rules={[{ required: true, message: 'Please provide a secret' }]} name="secrets">
                                    ...
                                    <ConditionalFormBlock dependency="credentials" condition={() => form.getFieldValue("credentials") === "direct"}>
                                        <Alert
                                            ...
                                            message="Prerequisite"
                                            description={<div> "Make sure that you have stored the credentials file at <b> /root/.aws/credentals </b> of Space Cloud Gateway before saving this configuration. Space Cloud will drectly load the credentals from there.</div>}
                                            type="info"
                                            showIcon
                                        />
                                    </ConditionalFormBlock>
                                ...
                                <ConditionalFormBlock dependency="storeType" condition={() => form.getFieldValue("storeType") === "gcp-storage"}>
                                    ...
                                    <FormItemLabel name="Credentials File" description="Credentials file is used to authorize Space Cloud to your bucket" />
                                    <Form.Item name="credentials">
                                        <Radio.Group defaultValue="secrets">
                                            <Radio style={{ display: 'block' }} value="secrets">Load from Space Cloud Secrets <span style={{ fontWeight: 'bold' }}>(recommended method)</span></Radio>
                                            <Radio style={{ display: 'block', marginTop: "3%" }} className="radio-align" value="direct">Load Directly</Radio>
                                        </Radio.Group>
                                    </Form.Item>
                                    <ConditionalFormBlock dependency="credentials" condition={() => form.getFieldValue("credentials") === "secrets"}>
                                    ...
                                    <ConditionalFormBlock dependency="credentials" condition={() => form.getFieldValue("credentials") === "direct"}>
                                        <Alert
                                            style={{ marginBottom: "3%" }}
                                            message="Prerequisite"
                                            description={<div> Make sure that you have stored the credentials file in a path accessible to the Space Cloud Gateway and started the Gateway with the <b> GOOGLE_APPLICATION_CREDENTIALS </b> environment (pointing to the credentials file path) before saving this configuration. Space Cloud will load the credentials from the path specified in this environment variable </div>}
                                            type="info"
                                            showIcon
                                        />
                                    </ConditionalFormBlock>

...
const ConfigureEndpoint = () => {
  ...
  const serviceURL = useSelector((state) =>
    getRemoteServiceURL(state, serviceName)
  );

  useEffect(() => {
    ReactGA.pageview(
      `/projects/remote-services/endpoints/${endpointName ? "edit" : "add"}`
    );
  }, []);
  ...
  const handleSaveEndpoint = (
    ...
    saveRemoteServiceEndpoint(projectID, serviceName, name, endpointConfig)
      .then(({ queued }) => {
        notify(
          ...
          queued
            ? actionQueuedMessage
            : `${
                isEndpointPresent ? "Modified" : "Added"
      ...
      .catch((ex) =>
        notify(
          "error",
          `Error ${isEndpointPresent ? "modifying" : "adding"} endpoint`,
          ex
        )
      )
      ...
      <ProjectPageLayout>
        ...
        <Content>
          <EndpointForm
            ...
            initialValues={
              endpointName
                ? Object.assign({}, endpoints[endpointName], {
                    name: endpointName,
                  })
                : undefined
            }

...
const ServiceTopBar = ({ projectID, serviceName }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ marginLeft: 16 }}>Edit endpoint</span>
    </div>
  );
};
...
const EditEndpoint = () => {
  // Router params
  const { projectID, serviceName, endpoint } = useParams();
  ...
  const initialValues = getProjectConfig(
    projects,
    projectID,
    `modules.remoteServices.externalServices.${serviceName}.endpoints.${endpoint}`
  );
  ...
  return (
    <React.Fragment>
      ...
      <div className='page-content page-content--no-padding'>
        ...
        <EndpointForm 
          initialValues = {{...initialValues, name: endpoint}}

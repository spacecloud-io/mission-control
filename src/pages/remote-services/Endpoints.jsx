...
import {
  deleteRemoteServiceEndpoint,
  getRemoteServiceEndpoints,
} from "../../operations/remoteServices";
  ...
  return (
    ...
    >
      <Button
        type="link"
        onClick={() =>
          history.push(`/mission-control/projects/${projectID}/remote-services`)
        }
      >
        <LeftOutlined />
        Go back
      </Button>
      <span style={{ marginLeft: 16 }}>{serviceName}</span>
    </div>
  );
};
...
const RemoteService = () => {
  ...
  // Global state
  const endpoints = useSelector((state) =>
    getRemoteServiceEndpoints(state, serviceName)
  );

  // Derived state
  const endpointsTableData = Object.entries(
    endpoints
  ).map(([name, { path, kind, method }]) => ({ name, method, path, kind }));
    ...
    deleteRemoteServiceEndpoint(projectID, serviceName, name)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Removed endpoint successfully"
        )
      )
  ...
  const handleSecureClick = (endpoint) =>
    openSecurityRulesPage(
      projectID,
      securityRuleGroups.REMOTE_SERVICES,
      endpoint,
      serviceName
    );
  ...
  const tableColumns = [
    ...
    {
      title: "Method",
      render: (_, { kind, method }) =>
        kind === endpointTypes.PREPARED ? "POST" : method,
    },
    {
      title: "Path",
      render: (_, { kind, path }) =>
        kind === endpointTypes.PREPARED
          ? `http://localhost:4122/v1/api/${projectID}/graphql`
          : path,
    },
    {
      title: "Actions",
      ...
      render: (_, { name }) => (
        <span>
          <a
            onClick={() =>
              history.push(
                `/mission-control/projects/${projectID}/remote-services/${serviceName}/endpoints/${name}/edit`
              )
            }
          >
            Edit
          ...
          <Popconfirm
            title={`This will remove this endpoint from this service. Are you sure?`}
          ...
          {noOfEndpoints === 0 && (
            <div style={{ marginTop: 24 }}>
              <div className="panel">
                ...
                >
                  A service can have multiple endpoints that can be accessed
                  from the frontend.
                </p>
                <Button
                  ...
                  onClick={() =>
                    history.push(
                      `/mission-control/projects/${projectID}/remote-services/${serviceName}/endpoints/add`
                    )
                  }
                >
                  Add first endpoint

...
const RemoteServices = () => {
  ...
  // Derived state
  const servicesTableData = Object.entries(services).map(([name, { url }]) => ({
    name,
    url,
  }));
  const noOfServices = servicesTableData.length;
  const serviceClickedInfo = serviceClicked
    ? { name: serviceClicked, url: services[serviceClicked].url }
    : undefined;
  ...
  const handleSubmit = (name, url) => {
    return new Promise((resolve, reject) => {
      ...
      const newServiceConfig = Object.assign(
        {},
        serviceConfig ? serviceConfig : { endpoints: {} },
        { url }
      );
      incrementPendingRequests();
      saveRemoteService(projectID, name, newServiceConfig)
        ...
        .catch((ex) => {
          notify(
            "error",
            `Error ${serviceConfig ? "modifying" : "adding"} remote service`,
            ex
          );
          reject();
        })
  ...
  const handleViewClick = (name) => {
    history.push(
      `/mission-control/projects/${projectID}/remote-services/${name}`
    );
  };
    ...
    deleteRemoteService(projectID, name)
      .then(({ queued }) =>
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Removed remote service successfully"
        )
      )
      ...
      render: (_, { name }) => (
        <span style={{ display: "flex", justifyContent: "inline" }}>
          ...
          <div onClick={(e) => e.stopPropagation()}>
            <Popconfirm
              title={`This will remove this service and all its endpoints from Space Cloud. Are you sure?`}
              onConfirm={() => {
                handleDelete(name);
              }}
            >
        ...
        {noOfServices === 0 && (
          <div style={{ marginTop: 24 }}>
            <div className="panel">
              ...
              >
                Access your RESTful services via the unified GraphQL APIs of
                Space Cloud.{" "}
                <a href="https://docs.spaceuptech.com/microservices/graphql">
                  View Docs.
                </a>
              </p>
              <Button

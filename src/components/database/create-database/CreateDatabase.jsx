...
const CreateDatabase = (props) => {
  ...
  const formInitialValues = {
    ...
    conn: defaultDbConnectionStrings[dbTypes.MONGO],
    loadFromSecret: false,
  };
  ...
  const alertMsg = (
    <div>
      <b>Note:</b> If your database is running inside a docker container, use
      the container IP address of that docker container as the host in the
      connection string.
    </div>
  );
  ...
  return (
    <Card>
      ...
      >
        ...
        <FormItemLabel
          name="Provide a connection string"
          description="Space Cloud requires a connection string to connect to your database"
        />
        ...
        >
          <Form.Item
            name="conn"
            rules={[
              { required: true, message: "Please input a connection string" },
            ]}
          >
        ...
        >
          ...
          >
            <AutoComplete
              ...
              options={envSecrets.map((secret) => ({ value: secret }))}
            />
          </Form.Item>
        </ConditionalFormBlock>
        <Form.Item
          noStyle
          shouldUpdate={(prev, curr) => prev.dbType != curr.dbType}
          dependencies={["dbType"]}
        >
          {() => {
        ...
        <FormItemLabel
          name="Alias"
          description="Alias name is used in your frontend queries to identify your database"
        />
        <Form.Item
          name="alias"
          dependencies={["dbType"]}
          ...
          rules={[
            {
              validator: (_, value, cb) => {
                ...
                if (!/^[0-9a-zA-Z_]+$/.test(value)) {
                  cb(
                    "Alias name can only contain alphanumeric characters and underscores!"
                  );
                  return;
                }
                ...
                if (check && !props.ignoreDbAliasCheck) {
                  cb(
                    "Alias name already taken by another database. Please provide an unique alias name!"
                  );
                  return;
                }
                cb();
        ...
        <Form.Item
          shouldUpdate={(prev, curr) => prev.alias != curr.alias}
          dependencies={["alias"]}
        >
          {() => {
              ...
              return (
                <React.Fragment>
                  <FormItemLabel
                    ...
                    hint="Query articles tables (Note the alias directive):"
                  />
                  <CodeMirror
                    value={data}

...
function AlertMsgApplyTransformations() {
  return (
    <div>
      <b>Info</b> <br />
      Describe the transformed webhook request body using{" "}
      <a
    ...
    >
      <Form layout="vertical" form={form} initialValues={formInitialValues}>
          ...
          rules={[
            {
              validator: (_, value, cb) => {
                ...
                if (!/^[0-9a-zA-Z_]+$/.test(value)) {
                  cb(
                    "Trigger name can only contain alphanumeric characters and underscores!"
                  );
                  return;
                }
                cb();
        ...
        <ConditionalFormBlock
          dependency="source"
          condition={() => form.getFieldValue("source") === "database"}
        >
          ...
          <Input.Group compact>
            ...
            >
              <AutoComplete
                ...
                options={props.dbList.map((db) => ({ value: db }))}
              />
            </Form.Item>
            <Form.Item
              name={["options", "col"]}
        ...
        <ConditionalFormBlock
          dependency="source"
          condition={() => form.getFieldValue("source") === "file storage"}
        >
          <FormItemLabel name="Trigger operation" />
          <Form.Item
            name="type"
        ...
        <ConditionalFormBlock
          dependency="source"
          condition={() => form.getFieldValue("source") === "custom"}
        >
            ...
            rules={[
              {
                validator: (_, value, cb) => {
                  ...
                  if (!/^[0-9a-zA-Z_]+$/.test(value)) {
                    cb(
                      "Event type can only contain alphanumeric characters and underscores!"
                    );
                    return;
                  }
                  cb();
          ...
          >
            <Input
              placeholder="Custom event type (Example: my_custom_event_type)"
              defaultValue=""
            />
          </Form.Item>
        </ConditionalFormBlock>
        ...
        <Collapse style={{ background: "white" }} bordered={false}>
          <Collapse.Panel header="Advanced settings" key="advanced">
            ...
            >
              ...
              <FormItemLabel
                name="Template"
                description="Template to generate the transformed webhook request body."
              />
              <Form.Item

...
const FilterForm = (props) => {
  ...
  const triggerNames = useSelector((state) =>
    Object.keys(getEventingTriggerRules(state))
  );
  ...
  return (
    <Modal
      ...
      cancelButtonProps={{
        style: { float: "left" },
        onClick: () => {
          dispatch(reset("uiState.eventFilters"));
          props.handleCancel();
        },
      }}
    ...
    >
      <Form
        ...
        initialValues={{
          status: eventFilters.status
            ? eventFilters.status
            : ["processed", "failed", "staged"],
          showName: eventFilters.showName,
          showDate: eventFilters.showDate,
          name: eventFilters.name ? eventFilters.name : undefined,
          "range-picker": eventFilters.startDate
            ? [
                moment.unix(eventFilters.startDate, "YYYY-MM-DD"),
                moment.unix(eventFilters.endDate, "YYYY-MM-DD"),
              ]
      ...
      >
        ...
        <Form.Item name="status">
          <Checkbox.Group>
            <Checkbox value="processed">
              Processed <CheckOutlined style={{ color: "#00FF00" }} />
            </Checkbox>
            <Checkbox value="failed">
              Failed <CloseOutlined style={{ color: "red" }} />
            </Checkbox>
            <Checkbox value="staged">
        ...
        <ConditionalFormBlock
          dependency="showName"
          condition={() => form.getFieldValue("showName")}
        >
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please select atleast one event trigger",
              },
            ]}
          >
            <Select
              mode="tags"
              placeholder="Select event triggers for which you want to see the logs"
            >
              {triggerNames.map((val) => (
        ...
        <ConditionalFormBlock
          dependency="showDate"
          condition={() => form.getFieldValue("showDate")}
        >

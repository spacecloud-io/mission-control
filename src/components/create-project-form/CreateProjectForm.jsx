...
export default function CreateProjectForm({ projects = [], handleSubmit }) {
  ...
  const handleFormFinish = (values) =>
    handleSubmit(values.projectName, projectId);
  return (
    <Form
      form={form}
    ...
    >
        ...
        rules={[
          {
            validator: (_, value, cb) => {
              ...
              if (projects.some((project) => project === value.toLowerCase())) {
                cb(
                  "Project name already taken. Please provide a unique project name!"
                );
                return;
              }
              cb();
      ...
      <Form.Item>
        <Button type="primary" htmlType="submit" className="project-btn">
          Create project
        </Button>
      </Form.Item>
    </Form>

...
function InfiniteScrollingTable({
  id,
  hasMore,
  loadNext,
  scrollHeight = 720,
  ...
  const onScroll = (event) => {
    ...
    if (maxScroll === currentScroll) {
      if (!loading && globalState.hasMore && loadNext) {
        setLoading(true);
        loadNext(globalState.dataSource).finally(() => setLoading(false));
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (tableElement) {
      const tableBodyContainer = tableElement.querySelector(
        ".ant-table-container"
      );
      if (tableBodyContainer) {
        const tableBodyElement = tableBodyContainer.querySelector(
          ".ant-table-body"
        );
        if (tableBodyElement) {

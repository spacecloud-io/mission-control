...
const TeamManagementPermission = (props) => {
  ...
  return (
    <React.Fragment>
      <center>
        <img src={teamManagementSvg} height='50px' width='50px' />
        <SwapOutlined style={{ fontSize:'30px', margin:'0px 16px 0 16px' }}/>
        <img src={upLogo} height='45px' width='45px' />
        <h3>Team management requires the following permissions</h3>
      </center>
      <Divider />
      ...
      {permissions === 'configPermission' && 
        <Table style={{ marginTop:'24px' }} columns={configColumns} dataSource={configData} bordered scroll={{ y: 240 }} pagination={false} />}
      {permissions === 'apiPermission' && 
        <Table style={{ marginTop:'24px' }} columns={apiColumns} dataSource={apiData} bordered scroll={{ y: 240 }} pagination={false} />}
      <Button type='primary' style={{ width:'100%', marginTop:'32px' }} onClick={() => props.handleGrantPermission}>Grant permissions</Button>
    </React.Fragment>
  );
}

export default TeamManagementPermission;
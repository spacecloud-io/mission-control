...
const AESConfigure = ({ loading, aesKey, handleSubmit }) => {
  ...
  const handleClickGenerateKey = () =>
    form.setFieldsValue({ aesKey: generateAESKey() });
  ...
  return (
    <div>
      <h2>AES Key</h2>
      <p>
        This key is used by the security rules in Space Cloud to encrypt/decrypt
        certain fields
      </p>
      <Form form={form} initialValues={{ aesKey }} onFinish={handleFormSubmit}>
        ...
        <Form.Item shouldUpdate={(prev, curr) => prev.aesKey !== curr.aesKey}>
          {() => {
            const valueChanged =
              aesKey !== undefined && form.getFieldValue("aesKey") !== aesKey;
            return (

...
const generateRSAPublicKeyFromPrivateKey = (privateKey) => {
  ...
  var forgePublicKey = forge.pki.setRsaPublicKey(
    forgePrivateKey.n,
    forgePrivateKey.e
  );
...
const AddSecretModal = ({ handleSubmit, handleCancel }) => {
  ...
  const handleSubmitClick = (e) => {
    form.validateFields().then((values) => {
      ...
      values.publicKey =
        alg === "RS256" && privateKey
          ? generateRSAPublicKeyFromPrivateKey(privateKey)
          : values.publicKey;
    ...
    >
      ...
      >
        ...
        <ConditionalFormBlock
          dependency="alg"
          condition={() => form.getFieldValue("alg") === "HS256"}
        >
          ...
          <Form.Item name="isPrimary" valuePropName="checked">
            <Checkbox>
              Use this secret in user management module of API gateway to sign
              tokens on successful signup/signin requests
            </Checkbox>
          </Form.Item>
        </ConditionalFormBlock>
        <ConditionalFormBlock
          dependency="alg"
          condition={() => form.getFieldValue("alg") === "RS256"}
        >
          <FormItemLabel name="Private key" />
          <Form.Item name="privateKey">
            {isPrivateKeyLoading ? (
              <span>
                <Spin className="page-loading" spinning={true} size="large" />{" "}
        ...
        <ConditionalFormBlock
          dependency="alg"
          condition={() => form.getFieldValue("alg") === "RS256_PUBLIC"}
        >
...
const ViewSecretModal = ({ secretData, handleCancel }) => {
  return (
    <Modal
      ...
      width={720}
    >
      <Typography.Paragraph style={{ fontSize: 16 }} strong>
        Algorithm
      ...
      {secretData.alg === "HS256" && (
        <React.Fragment>
          <Typography.Paragraph
            style={{ fontSize: 16 }}
            copyable={{ text: secretData.secret }}
            strong
          >
            Secret
          </Typography.Paragraph>
      ...
      {secretData.alg === "RS256" && (
        <React.Fragment>
          <Typography.Paragraph
            style={{ fontSize: 16 }}
            copyable={{ text: secretData.publicKey }}
            strong
          >
            Public key
          </Typography.Paragraph>
          <div style={{ marginBottom: 24 }}>
            <Input.TextArea rows={4} value={secretData.publicKey} />
          </div>
          <Typography.Paragraph
            style={{ fontSize: 16 }}
            copyable={{ text: secretData.privateKey }}
            strong
          >
            Private key
          </Typography.Paragraph>
          <div style={{ marginBottom: 24 }}>
            <Input.TextArea rows={4} value={secretData.privateKey} />
          </div>
        </React.Fragment>
      )}
      {secretData.alg === "JWK_URL" && (
        <React.Fragment>
          <Typography.Paragraph
            style={{ fontSize: 16 }}
            copyable={{ text: secretData.jwkUrl }}
            strong
          >
            URL
          </Typography.Paragraph>
...
const SecretConfigure = ({
  secrets,
  handleRemoveSecret,
  handleChangePrimarySecret,
  handleAddSecret,
}) => {
  // For backword compatibility assume the `alg` as `HS256`
  const secretsData = secrets.map((obj) =>
    Object.assign({}, obj, { alg: obj.alg ? obj.alg : "HS256" })
  );
  ...
  const columns = [
    ...
    },
    {
      title: (
        <span>
          ...
          <Tooltip
            ...
            title="Primary secret is used by the user management module of API gateway to sign tokens on successful signup/signin requests."
          >
      ...
      render: (_, record, index) => {
        if (record.alg === "JWK_URL" || record.alg === "RS256_PUBLIC")
        ...
        return (
          <Radio
            ...
            onChange={
              !record.isPrimary
                ? () => handleChangePrimarySecret(index)
                : undefined
            }
          />
        );
        ...
        return (
          <span>
            ...
            <Popconfirm
              title={
                record.isPrimary
                  ? "You are deleting primary secret. Any remaining secret will be randomly chosen as primary key."
                  : "Tokens signed with this secret will stop getting verified"
              }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <Button onClick={() => setAddSecretModalVisible(true)}>Add</Button>
      </div>
      <p>
        These secrets are used by the auth module in Space Cloud to verify the
        JWT token for all API requests
      </p>
      <Alert

...
function formatPlan(plan) {
  ...
  return temp
    .split("-")
    .map((v) => capitalizeFirstCharacter(v))
    .join(" ");
}
  ...
  return (
    <React.Fragment>
      ...
      {!clusterUpgraded && (
        <React.Fragment>
          <p>
            No license applied. Apply a license key to operate Space Cloud in
            enterprise mode
          </p>
          <Space size="middle">
            <Button onClick={handleApplyLicense}>Apply license key</Button>
            <Button onClick={handleGetLicense}>
              Get a license <ArrowUpOutlined rotate={45} />
            </Button>
          </Space>
        </React.Fragment>
      )}
      {clusterUpgraded && (
        <React.Fragment>
          ...
          <Space size="middle" style={{ marginTop: 16 }}>
            <Button onClick={handleApplyLicense}>
              Apply a new license key
            </Button>
            <Button type="danger" onClick={handleRemoveLicense}>
              Remove license
            </Button>
          </Space>
        </React.Fragment>

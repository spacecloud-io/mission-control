...
function IntegrationsList({ integrations, showUploadCard }) {
    ...
    deleteIntegration(integratonId)
      .then(({ queued }) => {
        notify(
          "success",
          "Success",
          queued ? actionQueuedMessage : "Uninstalled integration successfully"
        );
      })
  ...
  const handleViewDetails = (integratonId) => {
    history.push(
      `/mission-control/projects/${projectID}/integrations/details/${integratonId}`
    );
  };

  const handleViewPermissions = (integratonId) => {
    history.push(
      `/mission-control/projects/${projectID}/integrations/permissions/${integratonId}`
    );
  };
  ...
  const handleUpload = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        history.push(
          `/mission-control/projects/${projectID}/integrations/install/${result.id}`,
          { useUploadedIntegration: true }
        );
      } catch (error) {
    ...
    <Row gutter={[24, 24]}>
      {showUploadCard && (
        <Col lg={{ span: 8 }}>
          ...
          >
            <center>
              <img src={uploadSvg} style={{ width: 96, height: 96 }} />

...
function SelectProject({ visible, handleCancel }) {
  ...
  const projects = useSelector((state) =>
    state.projects.map(({ id, name }) =>
      Object.assign({ id, name, selected: id === projectID })
    )
  );
  const columns = [
    {
      ...
      render: (_, record) => {
        return (
          <div>{record.selected && <CheckOutlined className="checked" />}</div>
        );
      },
  ...
  return (
    <div>
      <Modal
        ...
        footer={null}
        closable={false}
        bodyStyle={{ widtht: "800" }}
        title={
          <div className="modal-header">
            <h2 className="modal-title">Select a project</h2>
            <Button
              onClick={() => history.push("/mission-control/create-project")}
            >
              Create a project
            </Button>
          </div>

...
const convertRuleToGraph = (rule, id, parentId) => {
  ...
  if (rule.rule === "and" || rule.rule === "or") {
    if (rule.clauses) {
      rule.clauses.forEach((clause, index) => {
        graph = mergeGraph(
          graph,
          convertRuleToGraph(clause, `${id}.clauses.${index}`, id)
        );
      });
    }
  ...
  if (
    ...
    if (rule.clause && rule.clause.rule) {
      graph = mergeGraph(
        graph,
        convertRuleToGraph(rule.clause, `${id}.clause`, id)
      );
    } else {
      graph.nodes.push({
        id: `${id}.clause`,
  ...
  switch (ruleType) {
    case DB_COLLECTIONS:
      nodes.push({
        id: "root:create",
        label: "create",
        group: rule.create ? "root" : "no_rule",
      });
      edges.push({ from: "root", to: "root:create" });
      const dbCreateGraph = convertRuleToGraph(
        rule.create,
        "rule:create",
        "root:create"
      );
      ...
      nodes.push({
        id: "root:read",
        label: "read",
        group: rule.read ? "root" : "no_rule",
      });
      edges.push({ from: "root", to: "root:read" });
      const dbReadGraph = convertRuleToGraph(
        rule.read,
        "rule:read",
        "root:read"
      );
      ...
      nodes.push({
        id: "root:update",
        label: "update",
        group: rule.update ? "root" : "no_rule",
      });
      edges.push({ from: "root", to: "root:update" });
      const dbUpdateGraph = convertRuleToGraph(
        rule.update,
        "rule:update",
        "root:update"
      );
      ...
      nodes.push({
        id: "root:delete",
        label: "delete",
        group: rule.delete ? "root" : "no_rule",
      });
      edges.push({ from: "root", to: "root:delete" });
      const dbDeleteGraph = convertRuleToGraph(
        rule.delete,
        "rule:delete",
        "root:delete"
      );
    ...
    case FILESTORE:
      ...
      edges.push({ from: "root", to: "root:create" });
      const fileCreateGraph = convertRuleToGraph(
        rule.create,
        "rule:create",
        "root:create"
      );
      ...
      edges.push({ from: "root", to: "root:read" });
      const fileReadGraph = convertRuleToGraph(
        rule.read,
        "rule:read",
        "root:read"
      );
      ...
      edges.push({ from: "root", to: "root:delete" });
      const fileDeleteGraph = convertRuleToGraph(
        rule.delete,
        "rule:delete",
        "root:delete"
      );

...
const getStrippedKey = (ruleKey) =>
  ruleKey.replace("rule:", "").replace("root:", "").replace("root.", "");
...
function GraphEditor({ rule, setRule, ruleName, ruleMetaData }) {
  ...
  const doubleClickedRuleObj = doubleClickedNodeId
    ? getDoubleClickedRuleObject(rule, doubleClickedNodeId)
    : {}; // Used in the configure rule form as initial values
  ...
  const events = {
    ...
    doubleClick: function (event) {
      ...
      if (nodeId && nodeId.startsWith("root")) {
        message.error(
          "Operation not allowed. Only rule blocks (blue ones) can be double clicked to configure them"
        );
        return;
      }
  ...
  const shortcutsHandler = (key) => {
    ...
    const selectedRuleObj = Object.assign(
      {},
      strippedKey === "root" ? rule : dotProp.get(rule, strippedKey)
    );
    ...
    switch (key) {
      case "ctrl+c":
        if (selectedNodeId.includes("root:")) {
          message.error(
            "Copy is not allowed on this block. Copy a rule block instead"
          );
          return;
        }
      ...
      case "ctrl+x":
        if (strippedKey === "root" || selectedNodeId.includes("root:")) {
          message.error(
            "Cut is not allowed on this block. Only child rule blocks can be cut"
          );
          return;
        }
      ...
      case "alt+r":
        if (selectedNodeId.includes("root:")) {
          message.error(
            "Replace operation not allowed on this block. Try replacing a rule block (blue ones)"
          );
          return;
        }
      ...
      case "ctrl+v":
        getObjectFromLocalStorage(LOCAL_STORAGE_RULE_KEY)
          .then((copiedRule) => {
            ...
            if (
              selectedRuleObj.rule === "and" ||
              selectedRuleObj.rule === "or"
            ) {
              const noOfClauses = selectedRuleObj.clauses
                ? selectedRuleObj.clauses.length
                : 0;
              if (strippedKey === "root") {
                setRule(
                  dotProp.set(rule, `clauses.${noOfClauses}`, copiedRule)
                );
              } else {
                setRule(
                  dotProp.set(
                    rule,
                    `${strippedKey}.clauses.${noOfClauses}`,
                    copiedRule
                  )
                );
              }
            ...
            if (
              selectedRuleObj.rule === "query" ||
              selectedRuleObj.rule === "force" ||
              selectedRuleObj.rule === "remove" ||
              selectedRuleObj.rule === "encrypt" ||
              selectedRuleObj.rule === "decrypt" ||
              selectedRuleObj.rule === "hash"
            ) {
              if (strippedKey === "root") {

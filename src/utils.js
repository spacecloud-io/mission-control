...
export function canGenerateToken(state, projectId) {
  return checkResourcePermissions(
    state,
    projectId,
    [configResourceTypes.PROJECT_CONFIG],
    "modify"
  );
}
...
export const openSecurityRulesPage = (projectId, ruleType, id, group) => {
  const url = `/mission-control/projects/${projectId}/security-rules?ruleType=${ruleType}&id=${id}${
    group ? `&group=${group}` : ""
  }`;
...
export const openProject = (projectId) => {
  ...
  if (!doesExist) {
    ...
    notify(
      "info",
      "Info",
      "Opened another existing project as the requested project does not exist"
    );
  }
...
export function checkResourcePermissions(state, projectId, resources, verb) {
  ...
  return permissions.some((permission) => {
    const projectMatched = checkIfValueInArray(permission.project, [
      "*",
      projectId,
    ]);
    const resourceMatched = checkIfValueInArray(permission.resource, [
      "*",
      ...resources,
    ]);
    ...
    Promise.all(promises)
      .then(([projects]) => {
        ...
        if (!projectToBeOpened) {
          projectToBeOpened = localStorage.getItem("lastOpenedProject")
...
export function performSetup() {
  return new Promise((resolve, reject) => {
    ...
    setInterval(() => {
      ...
      refreshClusterTokenIfPresent().then(() => {
        // NOTE: loggedIn is always true in case of dev environment
        const loggedIn = isLoggedIn(store.getState());
...
function extractPathInfo(path) {
  ...
  return {
    projectId: pathValues[3],
    moduleId: pathValues[4] ? pathValues[4] : "overview",
  };
}
...
function usePagePermissions() {
  ...
  const hasPermissions = useSelector((state) =>
    checkModulePermissions(state, projectId, moduleId)
  );
  ...
  if (!hasPermissions) {
    ...
    const adjustedModuleId =
      moduleId === "no-permissions" ? queryParams.moduleId : moduleId;
    return {
      allowed: false,
      redirectUrl: `/mission-control/projects/${projectId}/no-permissions?moduleId=${adjustedModuleId}`,
    };
  }
    ...
    <Route
      ...
      render={(props) => {
        ...
        if (!isDbInConfig) {
          return (
            <Redirect
              to={`/mission-control/projects/${projectID}/database/${dbAliasNames[0]}`}
            />
          );
        }
...
export const setLastUsedValues = (projectId, newValue) => {
  ...
  localStorage.setItem(
    `lastUsedValues:${projectId}`,
    JSON.stringify(newLastUsedValues)
  );
};

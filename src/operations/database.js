    ...
    client.database
      ...
      .then((result = []) => {
        const dbPreparedQueries = result.reduce((prev, curr) => {
          ...
          const newPreparedQueryConfig = Object.assign(
            {},
            preparedQueryConfig,
            { id }
          );
...
const loadCollections = (projectId, dbAliasName) => {
  return new Promise((resolve, reject) => {
    client.database
      .listCollections(projectId, dbAliasName)
      .then((collections = []) => {
...
export const loadDBConnState = (projectId, dbAliasName) => {
  return new Promise((resolve, reject) => {
    client.database
      .getConnectionState(projectId, dbAliasName)
      .then((connected) => {
...
export const inspectColSchema = (projectId, dbAliasName, colName) => {
  return new Promise((resolve, reject) => {
    client.database
      .inspectColSchema(projectId, dbAliasName, colName)
      .then(({ queued }) => {
        if (!queued) {
...
export const saveColSchema = (projectId, dbAliasName, colName, schema) => {
  return new Promise((resolve, reject) => {
    client.database
      .modifyColSchema(projectId, dbAliasName, colName, schema)
      .then(({ queued }) => {
        if (!queued) {
...
export const saveColRule = (
  ...
  return new Promise((resolve, reject) => {
    ...
    client.database
      .setColRule(projectId, dbAliasName, colName, collectionRules)
      .then(({ queued }) => {
        if (!queued) {
          store.dispatch(
            set(`dbRules.${dbAliasName}.${colName}`, collectionRules)
          );
        }
        resolve({ queued });
      })
...
export const saveColRealtimeEnabled = (
  projectId,
  dbAliasName,
  colName,
  isRealtimeEnabled
  ...
  return new Promise((resolve, reject) => {
    const collectionRules = getCollectionRules(
      store.getState(),
      dbAliasName,
      colName
    );
    const newCollectionRules = Object.assign({}, collectionRules, {
      isRealtimeEnabled,
    });
    client.database
      .setColRule(projectId, dbAliasName, colName, newCollectionRules)
      .then(({ queued }) => {
        if (!queued) {
          store.dispatch(
            set(
              `dbRules.${dbAliasName}.${colName}.isRealtimeEnabled`,
              isRealtimeEnabled
            )
          );
        }
...
export const saveColSecurityRules = (
  projectId,
  dbAliasName,
  colName,
  securityRules
...
export const untrackCollection = (projectId, dbAliasName, colName) => {
  return new Promise((resolve, reject) => {
    client.database
      .untrackCollection(projectId, dbAliasName, colName)
      .then(({ queued }) => {
        if (!queued) {
...
export const deleteCollection = (projectId, dbAliasName, colName) => {
  return new Promise((resolve, reject) => {
    client.database
      .deleteCol(projectId, dbAliasName, colName)
      .then(({ queued }) => {
        if (!queued) {
          const collectionsList = get(
            store.getState(),
            `dbCollections.${dbAliasName}`,
            []
          );
          const newCollectionsList = collectionsList.filter(
            (col) => col !== colName
          );
          ...
          store.dispatch(del(`dbRules.${dbAliasName}.${colName}`));
          store.dispatch(
            set(`dbCollections.${dbAliasName}`, newCollectionsList)
          );
        }
        resolve({ queued });
      })
...
export const savePreparedQueryConfig = (
  projectId,
  dbAliasName,
  id,
  args,
  ...
  return new Promise((resolve, reject) => {
    const preparedQueryConfig = getDbPreparedQuery(
      store.getState(),
      dbAliasName,
      id
    );
    ...
    client.database
      .setPreparedQuery(projectId, dbAliasName, id, config)
      .then(({ queued }) => {
        if (!queued) {
...
export const savePreparedQuerySecurityRule = (
  projectId,
  dbAliasName,
  id,
  rule
...
export const deletePreparedQuery = (projectId, dbAliasName, id) => {
  return new Promise((resolve, reject) => {
    client.database
      .deletePreparedQuery(projectId, dbAliasName, id)
      .then(({ queued }) => {
        if (!queued) {
...
export const reloadDbSchema = (projectId, dbAliasName) => {
  return new Promise((resolve, reject) => {
    client.database
      .reloadSchema(projectId, dbAliasName)
      .then(({ queued }) => {
        if (!queued) {
...
export const modifyDbSchema = (projectId, dbAliasName) => {
  return new Promise((resolve, reject) => {
    ...
    client.database
      .modifySchema(projectId, dbAliasName, dbSchemaRequest)
...
const saveDbConfig = (projectId, dbAliasName, enabled, conn, type, dbName) => {
  return new Promise((resolve, reject) => {
    ...
    client.database
      .setDbConfig(projectId, dbAliasName, dbConfig)
      .then(({ queued }) => {
        if (!queued) {
  ...
  return new Promise((resolve, reject) => {
    ...
    saveDbConfig(projectId, dbAliasName, true, conn, dbType, dbName)
      .then(({ queued }) => {
        ...
        const hasPermissionToSaveRule = checkResourcePermissions(
          store.getState(),
          projectId,
          [configResourceTypes.DB_RULES],
          permissionVerbs.MODIFY
        );
        if (hasPermissionToSaveRule) {
          saveColRule(
            ...
            console.error(
              "Error setting default collection rule" + ex.toString()
            )
          );
        }

        const hasPermissionToCreatePrepareduery = checkResourcePermissions(
          store.getState(),
          projectId,
          [configResourceTypes.DB_PREPARED_QUERIES],
          permissionVerbs.MODIFY
        ...
        if (hasPermissionToCreatePrepareduery) {
          if (isPreparedQueriesSupportedForDbType(dbType)) {
            savePreparedQuerySecurityRule(
              ...
              console.error(
                "Error setting default prepared query rule" + ex.toString()
              )
            );
          }
        }
        ...
        const hasPermissionToConfigureEventing = checkResourcePermissions(
          store.getState(),
          projectId,
...
export const removeDbConfig = (projectId, dbAliasName) => {
  return new Promise((resolve, reject) => {
    client.database
      .removeDbConfig(projectId, dbAliasName)
      .then(({ queued }) => {
        if (!queued) {
...
export const changeDbName = (projectId, dbAliasName, dbName) => {
  return new Promise((resolve, reject) => {
    ...
    saveDbConfig(projectId, dbAliasName, true, conn, type, dbName)
      .then(() => {
        const hasPermissionToChangeSchema = checkResourcePermissions(
          store.getState(),
          projectId,
          [configResourceTypes.DB_SCHEMA],
          permissionVerbs.MODIFY
...
export const getDbConfig = (state, dbAliasName) =>
  get(state, `dbConfigs.${dbAliasName}`, {});
export const getDbName = (state, projectId, dbAliasName) =>
  get(getDbConfig(state, dbAliasName), "name", projectId);
export const getDbType = (state, dbAliasName) =>
  get(getDbConfig(state, dbAliasName), "type", dbAliasName);
export const getDbConnState = (state, dbAliasName) =>
  get(state, `dbConnState.${dbAliasName}`, false);
export const getCollectionSchema = (state, dbAliasName, colName) =>
  get(state, `dbSchemas.${dbAliasName}.${colName}`, "");
export const getDbSchema = (state, dbAliasName) =>
  get(state, `dbSchemas.${dbAliasName}`, {});
export const getDbSchemas = (state) => get(state, "dbSchemas", {});
export const getDbRules = (state, dbAliasName) =>
  get(state, `dbRules.${dbAliasName}`, {});
export const getDbPreparedQueries = (state, dbAliasName) =>
  get(state, `dbPreparedQueries.${dbAliasName}`, {});
export const getDbPreparedQuery = (state, dbAliasName, preparedQueryId) =>
  get(state, `dbPreparedQueries.${dbAliasName}.${preparedQueryId}`, {
    id: preparedQueryId,
    sql: "",
    args: [],
    rule: {},
  });
export const getDbDefaultPreparedQuerySecurityRule = (state, dbAliasName) =>
  get(state, `dbPreparedQueries.${dbAliasName}.default.rule`, {});
export const getDbDefaultCollectionSecurityRule = (state, dbAliasName) =>
  get(state, `dbRules.${dbAliasName}.default.rules`, {});
export const getCollections = (state, dbAliasName) =>
  get(state, `dbCollections.${dbAliasName}`, []);
export const getTrackedCollectionsInfo = (state, dbAliasName) => {
  ...
  const collectionsInfo = Object.entries(
    collections
  ).map(([colName, { schema, rule }]) =>
    Object.assign({}, { name: colName, schema, ...rule })
  );
  return collectionsInfo.filter(
    ({ name }) =>
      name !== "default" && name !== "invocation_logs" && name !== "event_logs"
  );
};
...
export const getTrackedCollections = (state, dbAliasName) => {
  ...
  const rules = getDbRules(state, dbAliasName);
  const collections = [
    ...new Set([...Object.keys(schemas), ...Object.keys(rules)]),
  ];
  return collections.filter(
    (col) =>
      col !== "default" && col !== "invocation_logs" && col !== "event_logs"
  );
};
...
export const getDbConnectionString = (state, dbAliasName) =>
  get(getDbConfig(state, dbAliasName), "conn", "");
export const getCollectionRules = (state, dbAliasName, colName) =>
  get(state, `dbRules.${dbAliasName}.${colName}`, {
    isRealtimeEnabled: false,
    rules: {},
  });
export const getCollectionSecurityRule = (state, dbAliasName, colName) =>
  get(state, `dbRules.${dbAliasName}.${colName}.rules`, {});
export const getPreparedQuerySecurityRule = (state, dbAliasName, id) =>
  get(state, `dbPreparedQueries.${dbAliasName}.${id}.rule`, {});
...
export const isPreparedQueriesSupportedForDbType = (dbType) => {
  return [dbTypes.POSTGRESQL, dbTypes.MYSQL, dbTypes.SQLSERVER].some(
    (value) => value === dbType
  );
};
export const setPreparedQueryRule = (dbAliasName, id, rule) =>
  store.dispatch(set(`dbPreparedQueries.${dbAliasName}.${id}.rule`, rule));
export const setColSecurityRule = (dbAliasName, colName, rule) =>
  store.dispatch(set(`dbRules.${dbAliasName}.${colName}.rules`, rule));
export const setDbCollections = (dbAliasName, collections) =>

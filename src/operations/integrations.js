    ...
    client.integrations
      ...
      .then(({ queued }) => {
        if (!queued) {
          ...
          const newInstalledIntegrations = [
    ...
    client.integrations
      ...
      .then(({ queued }) => {
        if (!queued) {
          const state = store.getState();
          const installedIntegrations = getInstalledIntegrations(state);
          const newInstalledIntegrations = installedIntegrations.filter(
            (obj) => obj.id !== integrationId
          );
          setInstalledIntegrations(newInstalledIntegrations);
        }
        resolve({ queued });
...
export function getIntegrationConfigPermissions(
  state,
  integrationId,
  useUploadedIntegration
) {
  const { configPermissions = [] } = getIntegrationDetails(
    state,
    integrationId,
    useUploadedIntegration
  );
  const adjustedConfigPermissions = configPermissions.map(
    ({ resources = [], verbs = [] }) => {
  ...
  return Object.entries(permissionsMap).map(([key, value]) =>
    Object.assign({}, value, { resource: key })
  );
}

export function getIntegrationAPIPermissions(
  state,
  integrationId,
  useUploadedIntegration
) {
  const { apiPermissions = [] } = getIntegrationDetails(
    state,
    integrationId,
    useUploadedIntegration
  );
  const adjustedApiPermissions = apiPermissions.map(
    ({ resources = [], verbs = [] }) => {
      if (resources.length === 1 && resources[0] === "*") {
...
export function getIntegrations(state) {
  ...
  const installedIntegrations = getInstalledIntegrations(state);
  const supportedIntegrationsMap = supportedIntegrations.reduce(
    (prev, curr) => {
      return Object.assign({}, prev, {
        [curr.id]: Object.assign({}, curr, { installed: false }),
      });
    },
    {}
  );
  const installedIntegrationsMap = installedIntegrations.reduce(
    (prev, curr) => {
      return Object.assign({}, prev, {
        [curr.id]: Object.assign({}, curr, { installed: true }),
      });
    },
    {}
  );
  const integrationIds = [
    ...new Set([
      ...Object.keys(supportedIntegrationsMap),
      ...Object.keys(installedIntegrationsMap),
    ]),
  ];
  return integrationIds.map((id) =>
    installedIntegrationsMap[id]
      ? installedIntegrationsMap[id]
      : supportedIntegrationsMap[id]
  );
}

    ...
    client.secrets
      ...
      .then(({ queued }) => {
        if (!queued) {
          ...
          const newSecrets = upsertArray(
            secrets,
            (obj) => obj.id === secretConfig.id,
            () => secretConfig
          );
...
export const deleteSecretKey = (projectId, secretId, key) => {
  return new Promise((resolve, reject) => {
    client.secrets
      .deleteSecretKey(projectId, secretId, key)
      .then(({ queued }) => {
        if (!queued) {
          const secrets = get(store.getState(), "secrets", []);
          const newSecrets = secrets.map((obj) => {
...
export const saveRootPath = (projectId, secretId, rootPath) => {
  return new Promise((resolve, reject) => {
    client.secrets
      .setRootPath(projectId, secretId, rootPath)
      .then(({ queued }) => {
        if (!queued) {
          const secrets = get(store.getState(), "secrets", []);
          const newSecrets = secrets.map((obj) => {

...
const saveProjectSetting = (projectId, key, value) => {
  return new Promise((resolve, reject) => {
    ...
    const newProjectConfig = Object.assign({}, oldProjectConfig, {
      [key]: value,
    });
    const newProjects = projects.map((obj) =>
      obj.id === projectId ? newProjectConfig : obj
    );
    client.projects
      .setProjectConfig(projectId, newProjectConfig)
      .then(({ queued }) => {
        if (!queued) {
          store.dispatch(set("projects", newProjects));
        }
...
export const saveContextTimeGraphQL = (projectId, contextTimeGraphQL) =>
  saveProjectSetting(projectId, "contextTimeGraphQL", contextTimeGraphQL);

export const saveDockerRegistry = (projectId, dockerRegistry) =>
  saveProjectSetting(projectId, "dockerRegistry", dockerRegistry);

export const saveAesKey = (projectId, aesKey) =>
  saveProjectSetting(projectId, "aesKey", aesKey);

export const addSecret = (projectId, values) => {
  const { isPrimary, alg } = values;
  const secrets = store.getState().projects.find((obj) => obj.id === projectId)
    .secrets;
  const oldSecrets = isPrimary
    ? secrets.map((obj) => Object.assign({}, obj, { isPrimary: false }))
    : secrets;
...
export const changePrimarySecret = (projectId, index) => {
  const secrets = store.getState().projects.find((obj) => obj.id === projectId)
    .secrets;
  const newSecrets = secrets.map((obj, i) =>
    Object.assign({}, obj, { isPrimary: i === index ? true : false })
  );
  return saveProjectSetting(projectId, "secrets", newSecrets);
};
...
export const removeSecret = (projectId, index) => {
  ...
  if (!primarySecretPresent && newSecrets.length > 0)
    newSecrets[0].isPrimary = true;
  return saveProjectSetting(projectId, "secrets", newSecrets);
};

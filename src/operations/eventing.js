...
export const loadEventingTriggers = (projectId) => {
  return new Promise((resolve, reject) => {
    const hasPermission = checkResourcePermissions(
      store.getState(),
      projectId,
      [configResourceTypes.EVENTING_TRIGGERS],
      permissionVerbs.READ
    );
    if (!hasPermission) {
...
export const saveEventingSecurityRule = (
  projectId,
  eventType,
  securityRule
) => {
  return new Promise((resolve, reject) => {
    client.eventing
      .setSecurityRule(projectId, eventType, securityRule)
      .then(({ queued }) => {
        if (!queued) {
...
export const deleteEventingSecurityRule = (projectId, eventType) => {
  return new Promise((resolve, reject) => {
    client.eventing
      .deleteSecurityRule(projectId, eventType)
      .then(({ queued }) => {
        if (!queued) {
...
export const saveEventingSchema = (projectId, eventType, schema) => {
  return new Promise((resolve, reject) => {
    client.eventing
      .setEventSchema(projectId, eventType, schema)
      .then(({ queued }) => {
        if (!queued) {
...
export const deleteEventingSchema = (projectId, eventType) => {
  return new Promise((resolve, reject) => {
    client.eventing
      .deleteEventSchema(projectId, eventType)
      .then(({ queued }) => {
        if (!queued) {
...
export const saveEventingConfig = (projectId, enabled, dbAliasName) => {
  return new Promise((resolve, reject) => {
    ...
    client.eventing
      .setEventingConfig(projectId, eventingConfig)
      .then(({ queued }) => {
        if (!queued) {
          setEventingConfig(eventingConfig);
        }
        ...
        const hasPermissionToSetEventingRule = checkResourcePermissions(
          store.getState(),
          projectId,
          [configResourceTypes.EVENTING_RULES],
          permissionVerbs.READ
        ...
        if (enabled && hasPermissionToSetEventingRule) {
          const defaultEventingSecurityRule = get(
            store.getState(),
            "eventingRules.default",
            {}
          );
          const defaultEventingSecurityRuleExists =
            Object.keys(defaultEventingSecurityRule).length > 0;
          if (!defaultEventingSecurityRuleExists) {
            saveEventingSecurityRule(
              projectId,
              "default",
              defaultEventRule
            ).catch((ex) =>
              console.error(
                "Error setting default eventing rule while setting eventing config" +
                  ex.toString()
              )
            );
          }
...
export const saveEventingTriggerRule = (
  projectId,
  triggerName,
  type,
  url,
...
export const triggerCustomEvent = (
  ...
  return new Promise((resolve, reject) => {
    ...
    client.eventing
      .queueEvent(projectId, eventBody, token)
...
export const getEventingConfig = (state) =>
  get(state, "eventingConfig", { enabled: false, dbAlias: "" });
export const getEventingDbAliasName = (state) =>
  get(state, "eventingConfig.dbAlias", "");
export const getEventingTriggerRules = (state) =>
  get(state, "eventingTriggers", {});
export const getEventingSchemas = (state) => get(state, "eventingSchemas", {});
export const getEventingSecurityRule = (state, eventType) =>
  get(state, `eventingRules.${eventType}`, {});
export const getEventingSecurityRules = (state) =>
  get(state, "eventingRules", {});
export const getEventingDefaultSecurityRule = (state) =>
  get(state, "eventingRules.default", {});
export const setEventingSecurityRule = (eventType, rule) =>
  store.dispatch(set(`eventingRules.${eventType}`, rule));
const setEventingConfig = (eventingConfig) =>
  store.dispatch(set("eventingConfig", eventingConfig));
const setEventingRules = (eventingRules) =>
  store.dispatch(set("eventingRules", eventingRules));
const setEventingSchemas = (eventingSchemas) =>
  store.dispatch(set("eventingSchemas", eventingSchemas));
const setEventingTriggers = (eventingTriggers) =>

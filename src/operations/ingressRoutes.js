    ...
    client.routing
      ...
      .then(({ queued }) => {
        if (!queued) {
          ...
          const newIngressRoutes = upsertArray(
            ingressRoutes,
            (obj) => obj.id === routeId,
            () => newConfig
          );
...
export const saveIngressRouteRule = (projectId, routeId, rule) => {
  return new Promise((resolve, reject) => {
    ...
    client.routing
      .setRoutingConfig(projectId, routeId, newConfig)
      .then(({ queued }) => {
        if (!queued) {
    ...
    client.routing
      ...
      .then(({ queued }) => {
        if (!queued) {
          ...
          const newIngressRoutes = ingressRoutes.filter(
            (obj) => obj.id !== routeId
          );
          setIngressRoutes(newIngressRoutes);
        }
...
export const saveIngressGlobalRequestHeaders = (projectId, headers) => {
  return new Promise((resolve, reject) => {
    ...
    client.routing
      .setRoutingGlobalConfig(projectId, newGlobalConfig)
      .then(({ queued }) => {
        if (!queued) {
...
export const setIngressRoutes = (routes) =>
  store.dispatch(set(`ingressRoutes`, routes));

export const setIngressRouteRule = (routeId, rule) => {
  const ingressRoutes = getIngressRoutes(store.getState());
  const newIngressRoutes = ingressRoutes.map((obj) =>
    obj.id === routeId ? Object.assign({}, obj, { rule }) : obj
  );
  setIngressRoutes(newIngressRoutes);
};
...
export const getIngressRoutesGlobalConfig = (state) =>
  get(state, "ingressRoutesGlobal", {});
const setIngressRoutesGlobalConfig = (config) =>
  store.dispatch(set("ingressRoutesGlobal", config));

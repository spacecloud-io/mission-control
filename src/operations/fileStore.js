      ...
      .then(({ queued }) => {
        if (!queued) {
          ...
          if (getFileStoreRules(store.getState()).length === 0) {
            saveFileStoreRule(projectId, "DefaultRule", {
              prefix: "/",
...
export const saveFileStoreRule = (projectId, ruleName, rule) => {
  return new Promise((resolve, reject) => {
    ...
    client.fileStore
      .setRule(projectId, ruleName, fileRule)
      .then(({ queued }) => {
        if (!queued) {
          const fileStoreRules = get(store.getState(), "fileStoreRules", []);
          const newFileStoreRules = upsertArray(
            fileStoreRules,
            (obj) => obj.id === ruleName,
            () => fileRule
          );
...
export const saveFileStoreSecurityRule = (
  projectId,
  ruleName,
  securityRule
) => {
  ...
  return new Promise((resolve, reject) => {
    client.fileStore
      ...
      .then(({ queued }) => {
        if (!queued) {
          const fileStoreRules = get(store.getState(), "fileStoreRules", []);
          const newFileStoreRules = fileStoreRules.filter(
            (obj) => obj.id !== ruleName
          );
          setFileStoreRules(newFileStoreRules);
        }
...
const setFileStoreConnState = (connected) =>
  store.dispatch(set("fileStoreConnState", connected));
export const getFileStoreConnState = (state) =>
  get(state, "fileStoreConnState", false);
const setFileStoreRules = (rules) =>
  store.dispatch(set("fileStoreRules", rules));
export const setFileStoreSecurityRule = (ruleId, securityRule) => {
  ...
  const newRules = rules.map((obj) => {
    if (obj.id === ruleId)
      return Object.assign({}, obj, { rule: securityRule });
    return obj;
  });
  setFileStoreRules(newRules);
};

const setFileStoreConfig = (fileStoreConfig) =>

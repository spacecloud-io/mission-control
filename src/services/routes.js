class Routes {
  ...
  fetchIngressRoutes(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/routing/ingress`)
        .then(({ status, data }) => {
  ...
  fetchIngressRoutesGlobalConfig(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/routing/ingress/global`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  setRoutingConfig(projectId, routeId, config) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(
          `/v1/config/projects/${projectId}/routing/ingress/${routeId}`,
          { ...config }
        )
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
  ...
  setRoutingGlobalConfig(projectId, config) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(
          `/v1/config/projects/${projectId}/routing/ingress/global`,
          config
        )
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
  ...
  deleteRoutingConfig(projectId, routeId) {
    return new Promise((resolve, reject) => {
      this.client
        .delete(`/v1/config/projects/${projectId}/routing/ingress/${routeId}`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;

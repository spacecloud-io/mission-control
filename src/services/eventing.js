...
class Eventing {
  ...
  fetchEventingConfig(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/eventing/config`)
        .then(({ status, data }) => {
  ...
  fetchEventingSchemas(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/eventing/schema`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchEventingRules(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/eventing/rules`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchEventingTriggers(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/config/projects/${projectId}/eventing/triggers`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchEventLogs(
    projectId,
    { status, showName, name, showDate, startDate, endDate },
    lastEventDate,
    dbType,
    getToken
  ) {
    ...
    return new Promise((resolve, reject) => {
        ...
        query {
          event_logs (
            ...
            where: {
              ...
              ${
                showName
                  ? "rule_name: {_in: $name, _regex: $regexForInternalEventLogs}"
                  : "rule_name: {_regex: $regexForInternalEventLogs}"
              }
      ...
      `,
          variables: {
            ...
            endDate,
            lastEventDate,
            regexForInternalEventLogs: `^(?!realtime-${dbType}-).*`,
          },
        })
        .then((res) => {
  ...
  setEventingConfig(projectId, config) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(
          `/v1/config/projects/${projectId}/eventing/config/eventing-config`,
          config
        )
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
  ...
  queueEvent(projectId, event, token) {
    ...
    return new Promise((resolve, reject) => {
      client
        .postJSON(`/v1/api/${projectId}/eventing/queue`, event, token)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  setTriggerRule(projectId, triggerName, triggerRule) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(
          `/v1/config/projects/${projectId}/eventing/triggers/${triggerName}`,
          { id: triggerName, ...triggerRule }
        )
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
  ...
  deleteSecurityRule(projectId, type) {
    return new Promise((resolve, reject) => {
      this.client
        .delete(`/v1/config/projects/${projectId}/eventing/rules/${type}`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  setSecurityRule(projectId, type, rule) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(`/v1/config/projects/${projectId}/eventing/rules/${type}`, {
          ...rule,
          id: type,
        })
        .then(({ status, data }) => {
  ...
  setEventSchema(projectId, type, schema) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(`/v1/config/projects/${projectId}/eventing/schema/${type}`, {
          id: type,
          schema: schema,
        })
        .then(({ status, data }) => {
  ...
  deleteEventSchema(projectId, type) {
    return new Promise((resolve, reject) => {
      this.client
        .delete(`/v1/config/projects/${projectId}/eventing/schema/${type}`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;

...
class Deployments {
  ...
  setDeploymentConfig(projectId, serviceId, version, serviceConfig) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(
          `/v1/runner/${projectId}/services/${serviceId}/${version}`,
          serviceConfig
        )
        .then(({ status, data }) => {
  ...
  deleteDeploymentConfig(projectId, serviceId, version) {
    return new Promise((resolve, reject) => {
      this.client
        .delete(`/v1/runner/${projectId}/services/${serviceId}/${version}`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchDeployments(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/runner/${projectId}/services`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchDeploymentRoutes(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/runner/${projectId}/service-routes`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  fetchDeploymentStatus(projectId) {
    return new Promise((resolve, reject) => {
      this.client
        .getJSON(`/v1/runner/${projectId}/services/status`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  async fetchDeploymentLogs(
    ...
    const url = spaceCloudClusterOrigin
      ? spaceCloudClusterOrigin + logsEndpoint
      : logsEndpoint;
  ...
  setDeploymentRoutes(projectId, serviceId, routes) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(`/v1/runner/${projectId}/service-routes/${serviceId}`, {
          routes: routes,
        })
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {

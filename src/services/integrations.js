...
class Integrations {
  ...
  fetchSupportedIntegrations() {
    return new Promise((resolve, reject) => {
      this.spaceUpClient
        ...
        .then((res) => {
          const integrations =
            res.data && res.data.integrations ? res.data.integrations : [];
  ...
  fetchInstalledIntegrations() {
    return new Promise((resolve, reject) => {
      this.spaceCloudClient
        .getJSON(`/v1/config/integrations`)
        .then(({ status, data }) => {
  ...
  installIntegration(integrationConfig) {
    return new Promise((resolve, reject) => {
      this.spaceCloudClient
        .postJSON(`/v1/config/integrations`, integrationConfig)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  removeIntegration(integrationId) {
    return new Promise((resolve, reject) => {
      this.spaceCloudClient
        .delete(`/v1/config/integrations/${integrationId}`)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;

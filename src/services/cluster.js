class Cluster {
  ...
  setConfig(clusterConfig) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON(`/v1/config/cluster`, clusterConfig)
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  setOfflineClusterLicense(license) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON("/v1/config/offline-license", { license })
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
  ...
  refreshToken(token) {
    return new Promise((resolve, reject) => {
      ...
      this.client
        .getJSON("/v1/config/refresh-token")
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
  ...
  login(user, key) {
    return new Promise((resolve, reject) => {
      this.client
        .postJSON("/v1/config/login", { user, key })
        .then(({ status, data }) => {
          if (status < 200 || status >= 300) {
            reject(data.error);
            return;
